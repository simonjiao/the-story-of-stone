...
实现边界：Bridge 的 active binding 存储在 Manager/Postgres，Orchestrator 不再保留正式 session binding 内存入口。Open WebUI `message_id`/`user_message_id` 原本只用于 run 幂等；session message append 去重、nonce replay 防护、binding lifecycle audit 等 hardening 已转入 [ISSUE-009](ISSUE-009-agent-identity-bridge-hardening.md) 跟踪。

## 后续动作

本 issue 的身份/session bridge baseline 已完成。稳定设计已合并到 `docs/agent-platform-design/`，后续以 Agent Platform 设计文档为准，不再维护独立 Bridge 计划/清单文档。Bridge hardening 和正式环境复测不在本 issue 内宣告完成，转由 [ISSUE-009](ISSUE-009-agent-identity-bridge-hardening.md) 跟踪。常规回归仍包括 Open WebUI Function 管理接口可用性、Function secret 轮换、历史搜索和停止按钮可访问性；相关问题分别由其他 issue 跟踪。
