...
## Realtime / Responses / Redis 扩展定位

Gateway 新增实时能力时仍不是第 5 个 Agent。目标边界是：

1. 不接 OpenAI Realtime；
2. 不处理音频、ASR、TTS、VAD 或音频播放；
3. 端侧把语音转成文字后，通过 Gateway 自有 WebSocket JSON event 协议提交；
4. Gateway 提供 `/v1/responses` 作为新主接口，支持 stream、background、status、
   cancel 和 replay；
5. Gateway 提供 `/v1/realtime/ws` 作为一等实时文字入口；
6. `/v1/chat/completions` 继续作为 OpenAI-compatible 兼容入口；
7. Redis Streams 承担 job、response event、response state、worker retry 和断线恢复；
8. SSE 和 WS 都从同一条 response event stream 读取公开事件；
9. evidence package、reviewer、context、memory 和 audit 的正式事实仍在 Runtime/SQLite
   治理边界内；
