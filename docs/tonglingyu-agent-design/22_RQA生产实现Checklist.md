...
- 新增 `<deployment>/scripts/verify-tonglingyu-rqa-quality-gate.sh`。默认会运行
...
- `<deployment>/scripts/verify-tonglingyu-release-readiness.sh` 已把
...
- `<deployment>/scripts/verify-tonglingyu-strict-gateway.sh` 和 RQA quality gate 输出同一
...
- [x] `<deployment>/scripts/test-tonglingyu-release-readiness-contract.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-rqa-backup-restore-drill.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-rqa-api-contract.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-rqa-performance-budget.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-rqa-user-lifecycle.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-release-security.sh`
- [x] `<deployment>/scripts/remediate-tonglingyu-rqa-eval-artifacts.sh` dry-run/apply
      contract
- [ ] `<deployment>/scripts/verify-tonglingyu-release-readiness.sh`
- [x] `<deployment>/scripts/verify-tonglingyu-rqa-release-automation.sh` fail-closed path
- [x] `<deployment>/scripts/verify-tonglingyu-release-readiness-report.sh` fixture path
- [x] `<deployment>/scripts/test-openwebui-gateway-admin-action-contract.sh`
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-release-security.sh` 并接入
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-performance-budget.sh`
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-api-contract.sh`
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-user-lifecycle.sh`
  并接入 release readiness 必跑 gate。该 gate 会真实启动本地 Gateway，验证
...
- 2026-05-16 已把 `<deployment>/scripts/test-openwebui-gateway-admin-action-contract.sh`
...
- 2026-05-16 已新增 release automation wrapper
  `<deployment>/scripts/verify-tonglingyu-rqa-release-automation.sh`，强制串联
...
- 2026-05-16 已新增
  `<deployment>/scripts/prepare-tonglingyu-remote-security-evidence.sh`，完整远端 release
...
- 2026-05-16 以 digest image refs 和 fixture image scan 复跑
  ...
  browser review 未确认。
- 2026-05-16 已新增 `<deployment>/scripts/sync-tonglingyu-remote-release-tools.sh`：
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-remote-live-gates.sh`：
...
- 当时仍不能宣布 I 完成：生产镜像真实 scan artifact 已生成并绑定到 release
  ...
  仍未闭合。
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-remote-release-automation.sh`，
...
- 2026-05-16 已新增 `tonglingyu-rqa-lifecycle-v1` 和
  ...
  batch tombstone。
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-backup-restore-drill.sh`。
...
- 2026-05-16 已新增 `tonglingyu-gateway rqa-restore-canary` 和
  `<deployment>/scripts/prepare-tonglingyu-rqa-restore-canary.sh`：当 live DB 没有可用于
...
- 2026-05-16 live DB 发现旧 `sources` schema 缺 source metadata 列，已新增
  ...
  `<deployment>/scripts/remediate-tonglingyu-kb-source-metadata.sh`。远端执行前已备份
...
- 新增 `<deployment>/runbooks/tonglingyu-rqa-release-runbook.md`，覆盖 RQA release
...
- 新增 `<deployment>/scripts/verify-tonglingyu-release-ops-readiness.sh`，并接入
  ...
  fail-closed。
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-post-release-monitor.sh`：
...
- Runtime 和 Gateway 管理员状态更新已补 status-history audit：runtime audit 和
  ...
  幂等重复提交不会重复创建状态变更记录。
- 新增 `<deployment>/scripts/verify-tonglingyu-rqa-incident-capacity.sh`，并接入
  `verify-tonglingyu-release-readiness.sh` 的 required gate
  ...
  fail-closed。
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-capacity-load-evidence.sh`：
...
- 2026-05-16 已新增
  `<deployment>/scripts/verify-tonglingyu-rqa-capacity-load-smoke.sh`：它会实际执行
...
- 2026-05-16 已新增
  `<deployment>/scripts/verify-tonglingyu-rqa-live-capacity-load-smoke.sh`：它通过
...
- 2026-05-16 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-incident-audit-evidence.sh`：
...
- 2026-05-16 已将提交 `4f514d0` 部署到 `hhost` 并重启
  `tonglingyu-gateway`。远端 `.env` 已按部署侧维护的 env 备份流程完成两次备份；
...
- 2026-05-16 已完成 Open WebUI browser-side review 闭环：
  `<deployment>/scripts/ensure-openwebui-tonglingyu-model-access.sh` 在 live
