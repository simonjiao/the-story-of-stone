...
gate 输出已绑定可复核 eval artifact；当前本地 DB 因仍有 157 个 open
retrieval failures，真实 RQA quality gate 正确 fail-closed）
...
- contract smoke 已覆盖 synthetic passed report、降阈值篡改、open P0 failure
  ...
  复制 DB 上验证会输出 `expected_review_status`、`required_evidence_type` 和
  `required_type_required`，供 saved validator 重算。
- 仍不能宣布 Milestone F 完成：阈值运行时配置、RQA eval 行为配置与 strict
  live gate 的逐字段一致性校验还需要补齐。
