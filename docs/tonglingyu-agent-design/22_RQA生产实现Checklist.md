...
状态：进行中（2026-05-15；release readiness 已接入 canonical RQA quality gate，
saved report validator 已能阻断篡改后的 ready artifact；当前本地 DB 因仍有
157 个 open retrieval failures，真实 RQA quality gate 正确 fail-closed）
...
- gate 使用 Production 默认阈值：quality report coverage、expected evidence
  hit@8、required type、exact term、forbidden conclusion、reviewer status 均需
  100%，expected evidence denominator 必须大于 0，open retrieval failures
  必须为 0。
- 当前本地 DB 验证结果：`quality_summary.status=passed`，但
  ...
  RQA production-ready。
- 仍不能宣布 Milestone F 完成：阈值运行时配置、RQA eval 行为配置与 strict
  live gate 的逐字段一致性校验，以及 missing-report/open-P0 的独立 contract
  smoke 还需要补齐。

## Milestone G：saved report validator

状态：进行中（2026-05-15；validator 已接入 retrieval quality canonical gate，
并校验 ready artifact 的 RQA gate stdout、阈值、schema、KB/source digest 和
行为配置摘要；仍需补齐从 eval report 原始文件重算派生字段的验证）
...
- `verify-tonglingyu-release-readiness-report.sh` 的 canonical gate set 已新增
  ...
  success JSON、降低默认阈值、缺少 source/KB digest、缺少行为配置摘要或
  RQA gate 未通过，都会验证失败。
- contract smoke 已新增 RQA gate stdout 缺失和 threshold tamper 负向用例；
  原有 ready flag、missing gate、extra gate、secret-like value、browser review
  tamper 用例继续通过。
- 仍不能宣布 Milestone G 完成：validator 尚未从保存的 eval report 原始文件
  重算全部 quality gate 派生字段，也尚未实现对高基数字段列表的专门扫描规则。
