...
- 2026-05-16 已新增 `deploy/scripts/verify-tonglingyu-remote-live-gates.sh`：
  ...
  `data/tonglingyu/remote-live-gates/<run_id>/`。已对 `hhost` 生成
  `remote-live-20260516T014927Z-75846` artifact，model upstream、strict Gateway、
  Open WebUI Function 和 Open WebUI admin Action 均通过；该报告同时证明远端
  部署目录缺少当前 RQA release automation、capacity/load、incident/capacity、
  backup/restore、ops、post-release monitor 和 security 脚本。
...
- 2026-05-16 `hhost` 已有独立 remote live-gate artifact，证明当前远端基础
  model upstream、Open WebUI Function/Admin Action 和 strict Gateway 可用；但
  这些结果尚未由当前本地最新版 release automation 生成，也未绑定到 live
  release report。
