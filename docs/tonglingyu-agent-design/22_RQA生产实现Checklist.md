...
- 仍不能宣布 I 完成：`verify-tonglingyu-release-readiness.sh` 当前仍因本机缺
  docker 导致 `runtime_config` 失败，且 `security_scan` 缺真实 dependency/image
  scan 与 image digest；live gates 和 browser review 仍未执行。
