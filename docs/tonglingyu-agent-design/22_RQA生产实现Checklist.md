...
- 2026-05-16 已通过 release readiness contract 和本地 RQA backup/restore drill。
...
- 这只覆盖 release contract、恢复演练和安全门禁切片；runtime/gateway 全量
  cargo test、clippy、gateway smoke、真实 live release readiness、真实 scanner
  artifact 和 live/load 性能证据仍未完成。
...
- 2026-05-16 已修复 backup/restore drill 的嵌套 release report 边界，使其显式
  ...
  和 Open WebUI admin Action contract gates；恢复演练再次通过。
...
- 2026-05-16 security gate 已支持生产 digest-pinned image refs：
  ...
  refs 后再检查 mutable tag / digest missing。使用 fixture dependency/image scan
  和 digest image refs 的静态验证已通过；真实 scanner artifact 仍未生成。
- 2026-05-16 `runtime_config` 已支持非 live preflight 的静态 compose/env 解析；
  ...
  Docker Compose config。重新运行 `verify-tonglingyu-release-readiness.sh` 后，
  required failure 已收敛为 `security_scan`。
- 仍不能宣布 I 完成：`security_scan` 缺真实 dependency/image scan 与生产镜像
  digest；live gates 和 browser review 仍未执行。
...
- [ ] production report 引用的数据保留必须接入 release artifact lifecycle 或 report
      reference registry。
...
- [ ] 单测或 smoke 覆盖 backup/restore 和 live report freshness。
...
- `verify-tonglingyu-release-readiness.sh` 已把 `rqa_backup_restore_drill` 接为必跑
  ...
  post-restore checks 和 artifact hash。production-ready report 若使用 fixture
  source_mode 会被拒绝；live release 需要提供
...
- 仍不能宣布 J 完成或整体 RQA production-ready：本地 fixture 恢复演练、本地
  性能预算 gate 和本地 lifecycle gate 已闭合，但 live existing_refs 恢复证据、
  production report 引用保留、真实安全扫描 artifact 和 live/load 性能证据仍未
  闭合。
...
- Open WebUI admin Action source/fixture contract 已接入 release readiness 必跑
  ...
  secret 输出边界；live Open WebUI admin Action 验证、live/load 性能证据和值守
  证据仍未完成。
