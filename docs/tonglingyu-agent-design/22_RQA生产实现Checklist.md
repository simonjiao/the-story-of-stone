...
- 2026-05-16 已新增 `tonglingyu-gateway rqa-restore-canary` 和
  `deploy/scripts/prepare-tonglingyu-rqa-restore-canary.sh`：当 live DB 没有可用于
...
- 2026-05-16 live DB 发现旧 `sources` schema 缺 source metadata 列，已新增
  ...
  `deploy/scripts/remediate-tonglingyu-kb-source-metadata.sh`。远端执行前已备份
...
- 2026-05-16 已完成 Open WebUI browser-side review 闭环：
  `deploy/scripts/ensure-openwebui-tonglingyu-model-access.sh` 在 live
