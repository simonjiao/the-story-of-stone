...
- RQA API contract gate 已定义 `tonglingyu-rqa-api-contract-v1`，覆盖 admin list/read
  schema/pagination/max page、未知 filter、非法 enum filter、旧客户端解析、响应新增
  字段容忍、RQA admin mutation 未知字段拒绝和完整 prompt 泄露检查。兼容策略版本为
  `tonglingyu-rqa-api-compatibility-v1`：响应新增字段允许旧客户端忽略，query
  未知字段拒绝为 400，request body 未知字段拒绝为 422。
