...
- 2026-05-16 security gate 已支持生产 digest-pinned image refs：
  ...
  scan 和 digest image refs 的 security gate 已通过；image scan artifact 现在必须
  带 `scanned_image_refs_sha256` 并与当前 compose 解析出的 `image_refs_sha256`
  一致，saved report validator 会拒绝不匹配的 scan。生产镜像的真实 Trivy artifact
  仍未生成。
