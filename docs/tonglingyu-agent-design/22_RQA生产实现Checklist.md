...
- 2026-05-16 又补齐通灵玉铭文与青埂峰核心 eval target 的检索保护：
  通灵玉“字”问题强制 exact term 命中 `莫失莫忘` / `一除邪祟`，青埂问题
  强制 exact term 命中 `青埂`；exact-text fallback 优先
  `hongloumeng-wikisource-120` source snapshot，再回退到程甲/程乙。
- `tonglingyu-gateway eval` 默认改为在 SQLite 临时副本上运行，避免手动 eval
  把负向/降级用例登记进目标 DB 的 RQA 队列；只有显式
  `--allow-db-mutation` / `TONGLINGYU_EVAL_ALLOW_DB_MUTATION=true` 才允许写回。
...
- 验证命令：
  ...
  `python3 -m unittest open-webui/functions/test_tonglingyu_gateway_admin_action.py`
  （10 个测试通过）。
...
- 新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-quality-gate.sh`。默认会运行
...
- `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness.sh` 已把
...
- `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-strict-gateway.sh` 和 RQA quality gate 输出同一
...
- [x] `../tonglingyu-gatekeeper/deploy/scripts/test-tonglingyu-release-readiness-contract.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-backup-restore-drill.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-api-contract.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-performance-budget.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-user-lifecycle.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-security.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/remediate-tonglingyu-rqa-eval-artifacts.sh` dry-run/apply
      contract
- [ ] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness.sh`
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-release-automation.sh` fail-closed path
- [x] `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness-report.sh` fixture path
- [x] `../tonglingyu-gatekeeper/deploy/scripts/test-openwebui-gateway-admin-action-contract.sh`
...
- 2026-05-16 release readiness contract 已通过；此前本地 RQA backup/restore
  drill 通过过一次。持久 artifact 加固后真实脚本路径曾因恢复后 RQA eval
  case fail-closed；本轮修正 exact-term / source-priority 检索后，本地 eval
  重新达到 103/103，`expected_evidence_hit@8=5/5`，
  `exact_term_coverage=3/3`，真实 `existing_refs` restore drill 也已重新通过。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-security.sh` 并接入
...
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-performance-budget.sh`
...
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-api-contract.sh`
...
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-user-lifecycle.sh`
  并接入 release readiness 必跑 gate。该 gate 会真实启动本地 Gateway，验证
...
- 2026-05-16 已把 `../tonglingyu-gatekeeper/deploy/scripts/test-openwebui-gateway-admin-action-contract.sh`
...
- 2026-05-16 已修复 backup/restore drill 的嵌套 release report 边界，使其显式
  ...
  和 Open WebUI admin Action contract gates；contract 已覆盖该边界。本轮重新跑
  真实 `existing_refs` 路径后，恢复后 RQA quality gate、saved report validator、
  admin trace/failure/task/package 读取和 package replay 均通过。
...
- 2026-05-16 已新增 release automation wrapper
  `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-release-automation.sh`，强制串联
...
- 2026-05-16 已新增 `remediate-tonglingyu-rqa-eval-artifacts.sh`，默认 dry-run，
  ...
  本轮又审计关闭 25 个 `eval-tly-*` failure 和 25 个 governance tasks，备份为
  `data/tonglingyu/backups/rqa-eval-artifact-remediation-20260516T005451Z.db`，
  SHA-256 为
  `00426cf0fe5c17e8946e0704de4f82bfd9ab41c4d16fcaff5172957affdd8d22`。在 eval CLI
  默认 snapshot-copy 修正前，手动复测又产生 25 个 failure 和 25 个 governance
  tasks，已再次审计关闭；备份为
...
- 2026-05-16 security gate 已支持生产 digest-pinned image refs：
  ...
  refs 后再检查 mutable tag / digest missing。2026-05-17 仓库边界已删除旧
  Postgres/store/JWT 链路，security gate 只需要覆盖 Tonglingyu workspace 和当前
  compose image inventory。使用真实 `cargo-audit`、fixture image scan 和 digest
  image refs 的 security gate 已通过；真实 Trivy 路径会把
...
- 2026-05-16 已把 `agent-platform` 与 `tonglingyu-gateway` runtime image 切到
  ...
  image refs 为
  `sha256:c559d34b96a430bb53650906ce8a23f64948904c4f2af7028c6e588a8f537a77`
  和
  `sha256:084aa51d528359e6f86b3b574ebb59f4f7ddd72e4dda1adae0323190e6546bcb`，
  对应 Trivy raw report 为 0 critical / 0 high。
- 2026-05-16 已新增
  ...
  image inventory 和 6 个 per-image Trivy raw reports。最新 artifact
  `remote-release-20260516T050324Z-37074` 显示 dependency scan 0 critical /
  0 high，image refs 均 immutable 且 raw reports 可读取；但 aggregate image
  scan 仍 fail-closed：`critical_count=63`、`high_count=714`，来源为第三方
  `hermes-agent`、`open-webui`、`cloudflared` 和 `postgres` 镜像。
...
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/sync-tonglingyu-remote-release-tools.sh`：
  ...
  `agent-platform` 源码和 `resources` 到 `hhost`，不覆盖远端 `.env`；同时从
  运行中的 `tonglingyu-gateway` 容器复制 Gateway 二进制并写入
  `.tonglingyu-release-tool-env`，让远端无 Rust toolchain 时仍能运行当前 RQA
  gates。最新同步 artifact：
  `data/tonglingyu/remote-release-tools/remote-tools-20260516T032646Z-10903/remote-release-tools-sync.json`。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-remote-live-gates.sh`：
  当本机没有 Docker CLI 时，它通过 SSH 在远端部署目录执行 live gates，并把
  stdout JSON、stderr、gate JSON 和聚合报告保存到
  `data/tonglingyu/remote-live-gates/<run_id>/`。同步当前 release 工具、升级远端
  `tonglingyu_gateway_admin` Action、重建并重启 `tonglingyu-gateway:formal` 后，
  `hhost` 当前 Gateway 运行镜像 digest 为
  `sha256:a117351dd4436659b7f3d9abd3be43a95b6f06dbaa5b9b6325693b92c8f774b8`，
  最新 `remote-live-20260516T020657Z-81956` artifact 显示 model upstream、
  strict Gateway、Open WebUI Function 和 Open WebUI admin Action 均通过。
- 当时仍不能宣布 I 完成：生产镜像真实 scan artifact 已生成并绑定到 release
  automation；在旧策略下，第三方镜像 high/critical findings 仍计入
  `security_scan` blocker，且 browser review 和当前 release report 的人工复核证据
  仍未闭合。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-remote-release-automation.sh`，
  用于在本机无 Docker CLI 时通过 SSH 执行目标 `hhost` live release automation，
  并回收 release automation、release readiness 和 saved report validator
  artifact。最新 `remote-release-20260516T050324Z-37074` 显示 live automation 中
  `runtime_config`、`rqa_migration_preflight`、`retrieval_quality`、
  `rqa_backup_restore_drill`、`rqa_performance_budget`、`rqa_api_contract`、
  `rqa_user_lifecycle`、`openwebui_admin_action_contract`、`model_upstream_network`、
  `strict_gateway`、`openwebui_function` 和 `openwebui_admin_action` 均通过；
  contract smoke 和 saved report validator 已通过；目标 live DB open
  retrieval failures / governance tasks 为 0，
  `restore_ref_available=true`。
- 同一轮在旧策略下仍 fail-closed：`security_scan` 已有真实扫描但存在未审批
  high/critical findings，
  `release_ops_readiness` 缺 post-release monitor evidence，
  `rqa_incident_capacity` 缺 capacity/load 与 incident/audit evidence，
  `openwebui_browser_review` 未确认。`rqa_backup_restore_drill` 已用 live
  `existing_refs` 通过，restore drill 备份走容器执行，RTO/RPO 均满足目标；
  提交后复跑已确认 tracked worktree dirty blocker 消失；I 仍不能完成。
...
- [x] 目标 production DB migration 前已保存真实备份路径和 schema preflight 输出。
- [x] live release mode 必须生成真实 RQA quality gate，不接受 fixture-only report。
- [x] release report schema / saved report validator 已强制绑定 environment、
      generated_at 和有效期。
- [x] 目标 production-ready report 已绑定真实 live environment、generated_at
      和仍有效的有效期窗口。
- [x] release report schema / saved report validator 已强制绑定 runtime
      identity：当前运行镜像 inventory、代码版本和 migration 状态。
- [x] 目标 production-ready report 已绑定真实 live runtime identity artifact：
      当前运行镜像 digest、代码版本和 migration 状态。
- [x] production-ready report 必须绑定当前 live KB 的 source snapshot digest、
      KB build hash、kb_version 和 eval run id。
- [x] production-ready report 必须绑定当前 live KB 的 source license summary 和
      attribution summary。
- [x] production-ready report 必须包含 RTO/RPO、最近一次恢复演练证据和恢复后
      gate 结果。
- [x] production-ready report 必须包含依赖/镜像/发布脚本安全扫描摘要或已审批
      risk exception。
- [x] production-ready report 必须绑定当前 live Runtime profile、prompt、tool
...
- [x] live gate schema / saved report validator 已验证 RQA admin Action/API
      权限边界。
- [x] 目标 live Open WebUI admin Action/API 权限边界证据已生成并绑定到
      release report。
- [x] live gate schema / saved report validator 已验证 RQA metrics 和 Prometheus
      不泄露 query 原文或 secret。
- [x] 目标 live RQA metrics / Prometheus 隐私证据已生成并绑定到 release report。
...
- [x] live smoke 覆盖 backup/restore 和 live report freshness。
...
- 2026-05-16 已新增 `tonglingyu-rqa-lifecycle-v1` 和
  ...
  batch tombstone。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-backup-restore-drill.sh`。
...
- 2026-05-16 已新增 `tonglingyu-gateway rqa-restore-canary` 和
  `../tonglingyu-gatekeeper/deploy/scripts/prepare-tonglingyu-rqa-restore-canary.sh`：当 live DB 没有可用于
  restore drill 的 retrieval failure / governance task 引用时，脚本会先备份 DB，
  再写入一组 `restore_drill_canary` 引用，并立即将 failure 置为 `resolved`、
  governance task 置为 `closed` / `p1`，保证 open P0 仍为 0。`hhost` live DB
  已通过该路径生成 refs；后续 remote release automation 中
  `rqa_backup_restore_drill` 以 `source_mode=existing_refs` 通过，备份
  `execution_mode=docker`，RTO/RPO 均满足目标。
- 2026-05-17 当前最终远端 automation
  `remote-release-20260517T185847Z-39274` 已闭合 Milestone J 的目标 live
  production-ready 证据：`release-readiness.json` 为 `status=passed`、
  `production_release_ready=true`、`required_failures=[]`、`release_blockers=[]`；
  `release-readiness-validation.json` 为 `status=ok`、
  `production_release_ready=true`、`errors=[]`；`release-automation.json` 为
  `status=ok`、`production_ready=true`。
- 同一 run 绑定 `/home/simon/tonglingyu-home-deploy` 目标环境、
  `environment=hhost`、`target=tonglingyu-rqa`、source commit
  `d9d17bc27a1f93d51d59ec2500dacd3ed18229cf`、`tracked_dirty=false`、4 个运行镜像
  inventory、当前 live KB `source_snapshot_digest`、`kb_build_hash`、
  `kb_version=kb-019e34ad728a70728646513367bcc15a`、
  `eval_run_id=rqa-eval-16e7dfebf2e8a8e2`、source license summary digest、
  Runtime profile/prompt/tool/reviewer policy/model upstream/decoding 摘要。
- 同一 run 的 live capacity/load gate 和 post-release ops evidence 均通过：
  60 分钟 post-release monitor 从 `2026-05-17T19:11:08Z` 到
  `2026-05-17T20:11:09Z`，13 条样本全部 `status=ok`；open P0 retrieval
  failures / governance tasks 均为 0。`strict_gateway` 采用 bounded retry
  policy，最终报告中 `attempt_count=1`、`failed_attempt_count=0`、
...
- 2026-05-16 `hhost` 已同步当前 release 工具，升级 Open WebUI admin Action，
  并重建当前 `tonglingyu-gateway:formal`；最新独立 remote live-gate artifact
  证明当前远端 model upstream、Open WebUI Function/Admin Action 和 strict
  Gateway 可用。但这些结果尚未由完整 live release automation 生成，也未绑定到
  live release report。
- 2026-05-16 live DB 发现旧 `sources` schema 缺 source metadata 列，已新增
  `tonglingyu-gateway kb-source-metadata-backfill` 和
  `../tonglingyu-gatekeeper/deploy/scripts/remediate-tonglingyu-kb-source-metadata.sh`。远端执行前已备份
  live DB；容器内 CLI 只做 additive column / value backfill，不重建 KB，不删除
  package、failure、task 或 audit。backfill report 显示补齐
  `source_url`、`license`、`license_url`、`license_source_url`、`attribution`
  和 `usage_boundary`，5 个 source 更新后缺失值为 0；随后 live RQA quality gate
  在目标 DB snapshot 上通过。
- 仍不能宣布 J 完成或整体 RQA production-ready：本地
  migration preflight/runtime identity 机制、性能预算 gate、lifecycle gate 和
  `existing_refs` restore drill 已闭合；目标 production DB 的真实 pre-migration
  backup/preflight artifact、真实 live release_context/report artifact、真实 live
  runtime identity artifact、live admin Action/API 与 metrics privacy gate 的
  release-report 绑定、live KB/Runtime 绑定已经进入远端 automation 证据链；但
  live `existing_refs` 恢复证据、真实安全扫描 artifact、post-release monitor 和
  live/load/incident evidence 仍未闭合。
...
- Open WebUI admin Action source/fixture contract 已接入 release readiness 必跑
  ...
  secret 输出边界；`hhost` 独立 live Open WebUI admin Action gate 已通过，但
  当前版本的 live Action evidence 仍必须绑定进 live release report，不能只引用
  单独命令输出。
...
- [x] post-release 监控窗口至少覆盖一次真实 live gate 和一次 admin Action/API
      查询。
- [x] post-release 监控记录 operator、时间、环境、报告路径和结论。
...
- 新增 `../tonglingyu-gatekeeper/deploy/runbooks/tonglingyu-rqa-release-runbook.md`，覆盖 RQA release
...
- 新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-ops-readiness.sh`，并接入
  ...
  fail-closed。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-post-release-monitor.sh`：
...
- Saved report validator 已要求 production-ready report 绑定
  ...
  gate ref 和高基数告警标签篡改。
- 2026-05-16 最新 release evidence baseline
  `remote-release-20260516T074522Z-71051` 已生成并绑定目标环境 post-release ops
...
- Runtime 和 Gateway 管理员状态更新已补 status-history audit：runtime audit 和
  ...
  幂等重复提交不会重复创建状态变更记录。
- 新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-incident-capacity.sh`，并接入
  `verify-tonglingyu-release-readiness.sh` 的 required gate
  ...
  fail-closed。
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-capacity-load-evidence.sh`：
...
- 2026-05-16 已新增
  `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-capacity-load-smoke.sh`：它会实际执行
...
- 2026-05-16 已新增
  `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-live-capacity-load-smoke.sh`：它通过
...
- 2026-05-16 已新增 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-rqa-incident-audit-evidence.sh`：
...
- 2026-05-16 已完成 Open WebUI browser-side review 闭环：
  `../tonglingyu-gatekeeper/deploy/scripts/ensure-openwebui-tonglingyu-model-access.sh` 在 live
  Open WebUI DB 中确保 `tonglingyu` 模型 active 且普通用户 `user:*` 有 read grant；
  普通用户
...
- browser review 同时暴露 Open WebUI metadata prompt 污染 RQA 队列的问题。
  ...
  `open_failures=0`、`open_p0_tasks=0`。
- 最新 release evidence baseline
  `remote-release-20260516T074522Z-71051` 已生成 live capacity/load evidence 和
  60 分钟 post-release ops evidence。初次完整 automation 暴露第二次 readiness
  复核复用 migration backup path 的脚本问题，已在提交
  `ab9ce22d503c97daa2678529762f6da523b4eb8c` 修复为独立 backup/restore artifact
  path 并重同步远端；使用同一 artifact 目录复核后，`rqa_migration_preflight`、
  `rqa_incident_capacity`、`release_ops_readiness`、`openwebui_browser_review` 均
  `passed`，open P0 retrieval failures / governance tasks 均为 0。当时 required
  failures 只剩 `security_scan`。提交
  `8aea6545aa9917b850701c3239c2f018eb01bd35` 已将第三方镜像 findings 改为
