...
- 2026-05-16 security gate 已支持生产 digest-pinned image refs：
  ...
  scan 和 digest image refs 的 security gate 已通过；真实 Trivy 路径会逐个解析
  image report 的 HIGH/CRITICAL vulnerability，并要求 image scan artifact 带
  `scanned_image_refs_sha256` / `scanned_reports_sha256`，且与当前 compose 解析出的
  `image_refs_sha256` 一致。saved report validator 会拒绝不匹配的 scan。生产镜像的
  真实 Trivy artifact 仍未生成。
...
- 仍不能宣布 J 完成或整体 RQA production-ready：本地 fixture 恢复演练、本地
  migration preflight 机制、性能预算 gate 和 lifecycle gate 已闭合，但目标
  production DB 的真实 pre-migration backup/preflight artifact、live existing_refs
  恢复证据、live report freshness、当前运行镜像/代码/migration 状态、live
  KB/Runtime 绑定、真实安全扫描 artifact 和 live/load 性能证据仍未闭合。
