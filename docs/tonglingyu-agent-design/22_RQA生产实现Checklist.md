...
- [ ] retention / prune 与用户 delete/anonymize 和 legal hold 策略一致。
...
- `verify-tonglingyu-release-readiness.sh` 已把 `rqa_backup_restore_drill` 接为必跑
  ...
  `TASK_ID` 对应的真实恢复引用。
- 仍不能宣布 J 完成或整体 RQA production-ready：本地 fixture 恢复演练和本地
  性能预算 gate 已闭合，但 live existing_refs 恢复证据、production report 引用
  保留、真实安全扫描 artifact、live/load 性能证据和 lifecycle contract smoke
  仍未闭合。
...
- RQA API contract gate 已定义 `tonglingyu-rqa-api-contract-v1`，覆盖 admin list/read
  schema/pagination/max page、未知 filter、非法 enum filter 和完整 prompt 泄露检查；
  Gateway 边界已把非法 enum filter 从 runtime 500 修正为 400。
- export、delete/anonymize、legal hold、用户生命周期 audit event、Open WebUI admin
  Action contract、旧/新字段兼容 smoke 和 live/load 性能证据仍未完成。
