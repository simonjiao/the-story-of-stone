...
- 仍不能宣布 I 完成：生产镜像真实 scan artifact 已生成并绑定到 release
  automation，但第三方镜像存在未审批 high/critical findings；browser review
  和当前 release report 的人工复核证据仍未闭合。
...
- 同一轮仍 fail-closed：`security_scan` 已有真实扫描但存在未审批
...
- Saved report validator 已要求 production-ready report 绑定
  ...
  gate ref 和高基数告警标签篡改。
- 2026-05-16 最新远端 release automation 已正确识别
  `deploy/runbooks/tonglingyu-rqa-release-runbook.md`，runbook sections 和 alert
  policy 通过；live gate 剩余失败集中在 rollback / RTO-RPO / alert /
  post-release monitor / operator / post-release live gate / admin Action 证据缺失。
...
- 最新完整远端 release automation
  `remote-release-20260516T065506Z-59843` 已确认 `openwebui_browser_review` 通过，
  open P0 retrieval failures / governance tasks 均为 0。当前 required failures
  收敛为 `security_scan` 和 `release_ops_readiness`；另有
  `tracked worktree must be clean for live release`，需要本轮代码提交并重同步后复跑。
