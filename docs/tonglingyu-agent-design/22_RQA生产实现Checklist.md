...
- 仍不能宣布 I 完成：`verify-tonglingyu-release-readiness.sh` 当前仍因本机缺
  docker、open P0 RQA failure/task、缺真实 security scan / image digest、未执行
  live gates 和 browser review 而失败；全量 `docs/tonglingyu-agent-design/*.md`
  markdownlint 也仍有历史文档格式问题。
