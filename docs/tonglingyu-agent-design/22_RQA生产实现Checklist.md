...
- 这只覆盖 release contract、恢复演练和安全门禁切片；runtime/gateway 全量
  cargo test、clippy、gateway smoke、真实 live release readiness 和真实 scanner
  artifact 仍未完成。
...
- `verify-tonglingyu-release-readiness.sh` 已把 `rqa_backup_restore_drill` 接为必跑
  ...
  `TASK_ID` 对应的真实恢复引用。
- 仍不能宣布 J 完成或整体 RQA production-ready：本地 fixture 恢复演练已闭合，
  但 live existing_refs 恢复证据、production report 引用保留、安全扫描、性能预算
  和 lifecycle contract smoke 仍未闭合。
...
- export、delete/anonymize、legal hold、用户生命周期 audit event、API contract
  smoke、性能预算和 release report 性能证据仍未完成。
