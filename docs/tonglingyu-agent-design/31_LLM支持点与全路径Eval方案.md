...
4. 若某阶段需要新增持久化表或 migration，必须先在对应阶段 checklist 中写明 table、migration、rollback 和 fixture，再进入代码实现。
...
任一 optional LLM 节点失败时，必须按第 10.1 节降级矩阵处理。optional 节点不能把失败包装成正常事实回答。
...
以下登记记录已关闭的设计决策。后续实现必须按对应 contract 落地，不能把设计结论重新降级为待确认项。
...
| U4 | D4 | 可关闭 | 当前设计新增 `conversation_state_summary`；它不能作为事实源，不能进 evidence package，只能进入授权 projection，不作为 resolver 的 `used_context_refs`。 | 无未关闭决策；实现时必须落地 writer/loader/schema/projection/eval。 |
| U5 | D5 | 可关闭 | 当前设计引入 LLM suggested retrieval policy；LLM 只能输出 suggestion，最终 policy 必须由 deterministic patch 约束。 | 无未关闭决策；实现时必须落地 suggestion schema、patch rule、fallback 和 retrieval eval。 |
...
| U15 | S2-S7 | 可关闭 | 所有 LLM 能力使用统一 mode enum：`disabled`、`shadow`、`enforced`；生产默认 `disabled`，阶段 gate 才能提升。 | 使用第 10 节 flag matrix 和回滚命令。 |
...
生产默认值必须是 `disabled`。每个阶段只能把当前阶段能力提升到 `shadow` 或 `enforced`，不能一次打开多个后续阶段能力。
...
### 10.1 Provider / Runtime Failure Policy
