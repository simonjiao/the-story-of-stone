# 通灵玉 Gateway LLM 接入与全路径 Eval 设计
...
| Commentary Profile | Runtime profile 方向存在 | 脂批/版本 observation | 脂批与前八十回正文同属默认可用证据；refs 必须来自本地 evidence ids | S6 |
...
| 脂批问题 | 脂批、对应正文、版本 | 脂批是默认可用证据；后四十回仍需显式版本范围 |
...
```json
{
  ...
  "coverage_assessment": {
    "status": "passed|partial|insufficient",
...
| `rag_evidence.jsonl` | 20 | version boundary mismatch、later-forty without explicit scope、missing gold evidence |
...
|---|---|---|---|---|
| LLM Agent question normalizer | `TONGLINGYU_AGENT_ROLE_QUESTION_NORMALIZER_PROVIDER` | provider profile | 已完成真实 gate | provider 必须存在；输出强制 validator/enforced |
| LLM Agent conversation state writer | `TONGLINGYU_AGENT_ROLE_CONVERSATION_STATE_PROVIDER` | provider profile | 已完成真实 gate | provider 必须存在；输出强制 validator/enforced |
