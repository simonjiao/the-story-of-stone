...
状态：repo-local updated; current HEAD requires a new live release run。
...
automation 的历史闭环；在本轮把 question/context LLM Agent 默认改为 enforced 后，当前
HEAD 需要重新执行目标环境 full remote release automation，不能复用旧 run 作为当前版本证明。
...
- 正式 release readiness 已把 LLM release report 纳入 required gate、release manifest、artifact registry
  ...
  registry 缺失等负向场景。
- 历史目标环境 live gate、目标环境 release readiness 和 saved validator 已由
  `remote-release-20260520T203350Z-1275` 证明 `production_ready_proven=true`；该证据绑定旧
  Story commit / image，不得作为当前 HEAD 的完成证明。

当前 blocker：

1. 当前 HEAD 尚未完成目标环境证明：默认 enforced、profile model mapping 必填和
   rollback target enforced 的 gatekeeper 收紧后，必须重新跑 full remote release automation。
2. 后续 release 必须重新跑当次 full remote release automation，不能复用历史 artifact。
...
- 2026-05-20 已按当时版本规则部署 `0.1.13` 到 `hhost`：
  ...
  version label 为 `0.1.13`。远端 `.env` 更新前已按部署侧维护的 env
  备份流程完成备份；具体策略与路径由 `<deployment>/` 维护；
...
- `<deployment>/scripts/verify-tonglingyu-runtime-config.sh` 已补 compose 渲染配置 gate：
...
- `<deployment>/scripts/verify-tonglingyu-strict-gateway.sh` 已补运行态 Gateway gate：
  ...
  runtime tool result。
- `<deployment>/scripts/verify-tonglingyu-release-readiness.sh` 已补聚合发布 gate：
...
- `<deployment>/scripts/test-tonglingyu-release-readiness-contract.sh` 已补 release
...
- Open WebUI 已补 `tonglingyu_gateway_admin` Action Function：只读查询 Gateway
  ...
  readiness live gate。
- `<deployment>/scripts/test-openwebui-gateway-admin-action-contract.sh` 已升级为结构化
...
- release/readiness、runtime config、strict Gateway、Open WebUI Bridge Function
  ...
  缺失文件错误不泄露 env 值。
- 已补 `<deployment>/scripts/ensure-tonglingyu-gateway-env.sh`，用于在备份后生成缺失的
  ...
  生成值。
- 2026-05-11 已用目标 `.env` 先执行 `<deployment>/scripts/env-backup.sh backup`，
...
- 已新增 `<deployment>/scripts/verify-model-upstream-network.sh`，release readiness
...
- RQA Milestone F/G 已完成 release artifact 主干实现：
  新增 `<deployment>/scripts/verify-tonglingyu-rqa-quality-gate.sh`，并把
...
- `<deployment>/scripts/verify-tonglingyu-strict-gateway.sh` 已输出与 RQA quality gate 同
...
- RQA backup/restore drill 已接入 release readiness 必跑 gate：
  `<deployment>/scripts/verify-tonglingyu-rqa-backup-restore-drill.sh` 会执行 DB backup、
...
- Release security gate 已接入 release readiness 必跑路径：
  `<deployment>/scripts/verify-tonglingyu-release-security.sh` 会记录 dependency scan、
...
- RQA performance budget gate 已接入 release readiness 必跑路径：
  `<deployment>/scripts/verify-tonglingyu-rqa-performance-budget.sh` 会启动本地 Gateway，
...
- RQA API contract gate 已接入 release readiness 必跑路径：
  `<deployment>/scripts/verify-tonglingyu-rqa-api-contract.sh` 会启动本地 Gateway，验证
...
- RQA 用户数据生命周期 gate 已接入 release readiness 必跑路径：
  `<deployment>/scripts/verify-tonglingyu-rqa-user-lifecycle.sh` 会启动本地 Gateway，验证
...
- RQA Milestone L 已新增发布值守 gate：
  `<deployment>/runbooks/tonglingyu-rqa-release-runbook.md` 记录 release flow、
  ...
  `<deployment>/scripts/verify-tonglingyu-release-ops-readiness.sh` 接入
...
- RQA Milestone M 的本地 incident/capacity gate 已开始落地：
  ...
  `<deployment>/scripts/verify-tonglingyu-rqa-incident-capacity.sh` 并接入 release
...
- 已新增 `<deployment>/scripts/verify-tonglingyu-rqa-release-automation.sh` 作为 RQA
...
- 已新增 `<deployment>/scripts/remediate-tonglingyu-rqa-eval-artifacts.sh` 处理旧版
...
- 2026-05-16 为 live restore drill 新增闭环 canary 路径：
  ...
  `<deployment>/scripts/prepare-tonglingyu-rqa-restore-canary.sh` 会先备份 live DB，host
...
- 2026-05-16 远端 live DB 暴露出旧 KB schema：`sources` 缺
  ...
  `<deployment>/scripts/remediate-tonglingyu-kb-source-metadata.sh`，并在 `hhost` 容器内
	  执行 additive backfill；备份归档在 远端 evidence artifact 中。
...
- Post-release monitor 已有可复核 evidence 机制：
  `<deployment>/scripts/verify-tonglingyu-post-release-monitor.sh` 会生成
...
- Capacity/load smoke 已有可复核 evidence 机制：
  `<deployment>/scripts/verify-tonglingyu-rqa-capacity-load-evidence.sh` 会生成
...
- Capacity/load smoke 现在已有真实执行 runner：
  `<deployment>/scripts/verify-tonglingyu-rqa-capacity-load-smoke.sh` 会实际运行本地
...
- 2026-05-16 已新增目标环境 live runner
  `<deployment>/scripts/verify-tonglingyu-rqa-live-capacity-load-smoke.sh`，并接入
...
- 已定位 live `rqa_write_p95_ms` 超预算的主要代码风险：RQA 写请求会先完成本地
  ...
  `cargo fmt --check` 和 `<deployment>/scripts/test-tonglingyu-release-readiness-contract.sh`。
  2026-05-16 已将提交 `4f514d0` 同步到 `hhost`，重建并重启
  `tonglingyu-gateway`；远端 `.env` 先按部署侧维护的 env 备份流程完成备份，
...
- 2026-05-16 已修复 Open WebUI 普通用户模型可见性：新增
  `<deployment>/scripts/ensure-openwebui-tonglingyu-model-access.sh`，在 live Open WebUI
  DB 中确保 `model:tonglingyu` active 且存在 `access_grant user:* read`。远端执行
  结果为 `public_read_grant_count=1`，普通用户内部 `/api/models` 验证
  `has_tonglingyu=true`。
- 同日已完成 Open WebUI browser-side review evidence，并在按部署侧维护的
...
- 已将 metadata 隔离修复部署到 `hhost`。远端 `.env` 先按部署侧维护的
  env 备份流程完成备份，
...
- 最终 saved report validator 为 `status=ok`、
  ...
  `status=ok`、`production_ready=true`，证据 artifact 由 release
  流程归档。
  因此通灵玉 RQA production-ready release gate 已在当前 run 中闭合。
- Incident drill / audit-history 已有可复核 evidence 机制：
  `<deployment>/scripts/verify-tonglingyu-rqa-incident-audit-evidence.sh` 会生成
...
15. 2026-05-20 已完成真实 LLM Agent question/context path 历史 release 闭环：
    ...
    validator 和 live capacity bounded retry 证据链。历史目标环境 release run
    ...
    `d5e0b17bfd4030495df0d97d67a0659cbf07af041682051feefd94fa8d66802a`。当前 HEAD
    已改为 question/context LLM Agent 默认 enforced，且 gatekeeper 会拒绝 profile
    model mapping 缺失或 rollback target 非 enforced；因此必须重跑 full remote
