...
5. 低风险 `user_private` 偏好的自动 promotion / read enablement 只能在 Phase 4
   打开；
...
6. card lifecycle：`approved candidate -> active memory_card`，仅限人工 `promote`；
...
6. scoped memory 不是 `user_private` 的降级实现。Phase 4 必须保留
...
7. memory 只能作为偏好、背景、工作方法、长期上下文摘要进入受控 context。它不能
   作为正式事实源，不能进入 evidence package，不能替代原文/脂批/版本证据，不能
   改变 reviewer 裁决；
8. Gateway 仍不能变成长期 memory 系统。Gateway 可以在当前实现阶段同进程执行
...
6. 命中自动策略时，由 actor=`memory_policy:auto` 写完整 transition audit；
