...
1. 可以进入 **Phase 1 Scoped Context 最小闭环实现**；
2. 不能声明 scoped context production-ready；
3. 不能声明 scoped memory production-ready；
4. 不能声明长期 memory、审核流、lifecycle 或 hhost 容量 gate 已闭合。

当前代码仍以 Open WebUI 会话映射、最后一条用户问题、Runtime workflow、
证据包和 reviewer 为主路径。本文定义下一阶段实现边界和验收要求。
...
7. 生产目标和禁止过早声明以 `21_实现目标与落地计划.md` 为准。
...
3. Runtime 调用前必须生成受控 `context_pack`；
...
当前运行路径不能被称为 scoped context：

1. `RuntimeWorkflowInput` 只有 `question`，没有 `context_pack` 或 session summary；
2. 正式答题路径主要取最后一条 user message；
3. `history_over_limit` 只记录截断事件，不生成可审计 session summary；
4. profile step message 只有 question、operation、output_ref 和 step output；
5. 没有 `interaction_context_id`、`context_pack_id`、`session_journal` 或 memory ACL；
6. Open WebUI metadata prompt 的特殊分支不等于正式上下文治理。

因此当前只能声明“单轮问题生产路径 + 会话映射已完成”。多轮上下文生产可用和长期
memory 生产可用都不能声明完成。
...
每次进入 Runtime profile 前动态生成。它不是长期事实源。

最低字段：

1. `context_pack_id`；
2. `trace_id`；
3. `interaction_context_id`；
4. `profile_name`；
5. `resolved_question`；
6. `session_summary`；
7. `active_scopes`；
...
9. `allowed_tools`；
10. `forbidden_tools`；
11. `memory_read_refs`，仅包含已授权、已审核、可读摘要；
12. `forbidden_context`；
13. `output_contract`；
14. `schema_version`。
...
8. ContextPackBuilder 为每个 profile 生成受控 `context_pack`；
9. Gateway 创建 Runtime step plan，并传入 context pack ref；
10. Runtime 执行 profile 和 read-only tools；
11. Runtime 生成 evidence package、draft、review result 和 final answer；
12. Gateway 写 `session_journal`；
13. Memory Collector 异步读取 journal 并生成 `memory_candidate`；
14. 审核或明确自动策略通过后才写 active `memory_card`；
15. Gateway 返回 OpenAI-compatible response。

Phase 1 只实现 1 到 12。Phase 3 才实现 13。Phase 4 才实现 14。

## Profile 可见性

<!-- markdownlint-disable MD013 -->
| Profile | 可见上下文 | 不可见上下文 |
| --- | --- | --- |
...
1. `interaction_context_id`；
2. `context_pack_id`；
3. `scope_id`、`scope_graph`、`context_scope_binding`；
4. `memory_read_scopes`、`memory_write_scopes`；
5. `allowed_tools`、`forbidden_tools`；
6. `runtime_step_plan`；
7. `reviewer_off`、`disable_reviewer`；
8. 内部 profile 名称；
9. `session_journal` 内容；
10. `memory_card` 状态。

外部可以提供的只有自然语言、附件、Open WebUI identity metadata、conversation id 和
message id。所有业务 context、role、scope、tool policy 和 memory policy 必须由
Gateway / Context Governance 重新解析。
...
1. Runtime step input 增加 `context_pack_ref`；
2. profile step message 只读取自己的 context pack；
3. `honglou-main` 可见 `resolved_question`、必要 session summary 和证据包；
4. `honglou-text` / `honglou-commentary` 只见检索问题和工具约束；
5. `honglou-reviewer` 只见用户问题、草稿、证据包和回答策略；
6. Runtime audit 记录 context pack ref、profile、tool policy 和 output ref；
7. reviewer 裁决仍以 evidence package 和本地 review enforcement 为准。

退出条件：

1. profile 间未授权上下文不可见；
2. Hermes transcript 不替代业务 session；
3. context pack schema 不匹配时 workflow fail-closed；
4. replay 能重建 context、Runtime step、package 和 review 链。
...
2. 普通用户不能传入 context、scope、memory、tool 或 reviewer 控制字段；
3. 每个请求都有 `user_session_id`、`interaction_context_id`、`context_pack_id` 和
   journal ref；
4. `context_pack` 只包含当前 profile 允许看到的上下文；
5. 多轮追问能解析或 fail-closed；
6. 多用户同名 conversation 不串上下文；
7. admin trace 可按 trace/context/journal/package 回放；
8. public response 不暴露 context/journal/memory 内部 ID；
9. metrics 不包含高基数字段或用户原文；
10. hhost live gate 通过。
...
1. 设计决策已冻结；
2. 文档已达到 Phase 1 进入实现条件；
3. Phase 1 可以开始编码；
4. scoped context 仍未实现；
5. scoped memory 仍未实现；
6. 长期 memory、审核页面、独立 Context Governance 服务、lifecycle 和 live 容量
   gate 仍未闭合；
7. 任何 production-ready 结论都必须等对应 phase 的实现、测试和 hhost live gate
   通过后再声明。
