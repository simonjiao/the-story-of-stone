...
Phase 4 实现 Scoped Memory Production。Phase 4 不重新定义候选/卡片状态机；它在
Phase 3 已验证的 `memory_candidate` / `memory_card` / audit 基础上，打开受 ACL
约束的读取面、自动 promotion 和完整 memory lifecycle production gate。
...
1. `memory_card` 的 read enablement、ACL enforcement 和读取策略；
2. 自动 promotion policy，包括低风险 user preference 自动 promotion；
3. profile_common、knowledge_space、research_topic、source_collection 或共享 scope
   memory 审核路径；
4. ContextPackBuilder 读取当前 scope 和 ACL 授权的 active memory 摘要；
5. export、anonymize、legal hold 覆盖 memory candidate 和 memory card；
6. backup/restore 验证覆盖 context、journal、memory、package、review 链；
7. 可选审核页面，但只能调用同一套 admin-only API。
...
截至 Phase 2 production gate 完成时，可以声明：

1. 设计决策已冻结；
2. Phase 1 Scoped Context 已实现并通过当次 `hhost` production gate；
3. Phase 2 Context-aware Runtime 已实现并通过当次 `hhost` production gate；
4. `context_pack` + `context_projection` + registered consumer 模型进入生产路径；
5. scoped memory 仍未实现；
6. 长期 memory、Memory Collector、审核页面、独立 Context Governance 服务和 memory
   lifecycle 仍未闭合；
7. Phase 2 production-ready 结论来自 Phase 2 自己的本地测试、hhost live gate、
   full remote release automation 和 saved validator，不复用 Phase 1 结论；
