...
2. Phase 2 Context-aware Runtime 已实现，并已通过当次 `hhost` production gate；
3. 不能声明 scoped memory production-ready；
4. 不能声明长期 memory、Memory Collector、审核流或 memory lifecycle 已闭合。

当前代码已具备 Phase 1 scoped context 和 Phase 2 Context-aware Runtime：
`user_session`、`interaction_context`、`context_pack`、`context_projection`、
`session_journal` 和 `resolved_question` 已进入生产路径。Runtime profile 只能通过
受控 `context_projection_ref` 读取自己的上下文投影。`context_pack` 保持请求级父对象，
用于审计、回放和 projection 生成，不作为 Runtime profile 的直接输入。
...
Phase 1 scoped context 和 Phase 2 Context-aware Runtime 已完成当次生产门禁：
...
5. `hhost` live gate 已覆盖 Phase 2 consumer projection 隔离和 fail-closed 场景；
6. 完整远端 release automation 已证明 release readiness、saved validator 和 open P0
   retrieval/governance 检查通过。

因此当前可以声明 Phase 2 Context-aware Runtime production-ready。这个结论仍不能
扩展成长期 memory 生产可用，也不能声明 Memory Collector、memory candidate、active
memory、审核页面、Context Governance 独立服务或非 Hermes external agent 已完成。
...
长期 memory 的统一对象。Phase 1/2 不实现 `memory_card`。Phase 3 可以由审核
状态机写出 `memory_card`，但必须保持回答读取面关闭；Phase 4 才允许
...
15. 审核状态机可以把 approved candidate promote 为 `memory_card`，但 Phase 3
    固定为不可被回答读取；
16. Phase 4 在 ACL、lifecycle、backup/restore、capacity 和 live gate 闭合后，才允许
...
Phase 3 实现 14 和 15 的候选/状态机闭环，但不能打开读取面。Phase 4 才实现 16。
...
4. Memory Collector 允许 LLM 辅助抽取和完整审核状态机；自动 promotion 只能在
   Phase 4 打开，Phase 3 只允许人工 `promote` 写出不可读 `memory_card`。
...
5. 自动 promotion / read enablement 只能在 Phase 4 打开，并且必须按
...
8. Phase 3 的 `memory_card` 即使为 `active` 状态，也固定不可被回答读取；
9. Phase 4 才允许 ContextPackBuilder 读取当前 scope 和 ACL 授权的 active memory
   摘要。
...
2. `promote`：在 ACL、scope、retention 和 sensitivity 合法时，从 approved
   candidate 写出 active `memory_card`；Phase 3 固定 `read_enabled=false`；
3. `reject`：拒绝候选并记录原因；
4. `reclassify`：调整 scope、sensitivity 或 candidate type，并记录前后差异；
5. `expire`：让候选过期，不再进入审核队列；
6. `revoke`：撤销已存在的 active `memory_card`；Phase 4 读取面打开后，后续
...
6. card lifecycle：`approved candidate -> active memory_card`；Phase 3 仅限人工
   `promote`，Phase 4 允许人工 `promote` 或 `scoped-memory-policy-v1` 自动
   `auto_promote`；
7. card lifecycle：`active -> revoked`；
8. card lifecycle：`active -> expired`；
9. read enablement lifecycle：Phase 3 固定 `read_enabled=false`；Phase 4 才允许在
...
Phase 3 中，共享 scope、profile_common、source_collection 或会影响回答策略的
candidate 必须逐条人工审核。Phase 4 中，这些 candidate 按
...
Phase 4 实现 Scoped Memory Production。目标不是最小闭环、演示路径或只验证
...
Phase 4 不重新定义 Phase 3 已验证的候选/卡片状态机；它在
...
2. Phase 3 的 collector、candidate/card 状态机、admin-only CLI/API 和 audit 已在
...
6. Phase 4 默认 policy version 固定为 `scoped-memory-policy-v1`。policy mode 默认
...
7. scoped memory 不是 `user_private` 的降级实现。Phase 4 必须保留
...
截至 Phase 3 production gate 完成时，可以声明：

1. 设计决策已冻结；
2. Phase 1 Scoped Context 已实现并通过当次 `hhost` production gate；
3. Phase 2 Context-aware Runtime 已实现并通过当次 `hhost` production gate；
4. Phase 3 Memory Candidate workflow 已实现并通过当次 `hhost` production gate；
5. `session_journal`、Memory Collector、`memory_candidate`、`memory_card`、
   状态机、admin-only CLI/API 和 transition audit 已进入生产候选/审核路径；
6. active memory 读取、自动 read enablement、ContextPackBuilder memory 读取和
   scoped memory production-ready 仍未实现；
7. Phase 3 production-ready 结论来自 Phase 3 自己的本地测试、hhost live gate、
   full remote release automation 和 saved validator，不复用 Phase 1/2 结论；
8. scoped memory production-ready 必须等 Phase 4 退出条件全部通过后再
   声明；
9. 非 Hermes external agent 只是 schema 预留点，当前不能声明已接入。
