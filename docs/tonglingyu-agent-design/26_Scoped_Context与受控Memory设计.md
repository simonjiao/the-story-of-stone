...
当前代码已具备 Scoped Context Request Path、Context Projection Runtime、
Memory Candidate Workflow 和 Scoped Memory Production：
`user_session`、`interaction_context`、`context_pack`、`context_projection`、
`session_journal`、`resolved_question`、`memory_candidate`、`memory_card`、
`memory_policy_decisions` 和 `memory_read_refs` 已进入受控路径。Runtime profile 只能通过
受控 `context_projection_ref` 读取自己的上下文投影和授权 memory 摘要。`context_pack`
保持请求级父对象，用于审计、回放和 projection 生成，不作为 Runtime profile 的直接输入。
...
7. Context Projection Runtime 实现细化以
   `28_Context_Projection_Runtime_Checklist.md` 为准；
8. Memory Candidate Workflow 实现细化以
   `29_Memory_Candidate_Workflow_Checklist.md` 为准；
9. 当前目标和禁止过早声明以 `PROGRESS.md` 为准。
...
3. Runtime 调用前必须生成受控 `context_pack`；Context Projection Runtime 起还必须生成对应的
   `context_projection`；
4. 运行过程必须写 `session_journal`；
5. 长期 memory 只能由延迟 collector 从 journal 抽取、审核和沉淀；
6. 正式事实仍只来自 runtime DB、证据包、reviewer、外部发布记录、状态流转和
   action/audit log。

Gateway 可以保存 context/journal 索引、摘要、ref 和审计信息，但不能变成长期
memory 系统。Context Governance 的目标态是独立服务；Scoped Context Request Path 可以同进程实现，
但必须保持独立模块、独立 schema、独立 contract、独立 config、独立 audit 和独立
版本治理。

## 当前实现状态

Scoped Context Request Path、Context Projection Runtime、Memory Candidate Workflow 和
Scoped Memory Production 已完成各自当次生产门禁：

1. Runtime step input 已把 `context_pack_ref` 与 `context_projection_ref` 作为强制
   contract；
2. profile step message 已按 consumer projection 做强隔离；
3. Runtime audit 已记录 context pack ref、projection ref、consumer、runtime
   adapter、schema version、tool policy digest 和 output ref；
4. replay 已按历史 pack/projection 重建 context、Runtime step、package 和 review 链；
5. `hhost` live gate 已覆盖 Context Projection Runtime consumer projection 隔离和 fail-closed 场景；
6. Memory Collector、candidate/card 状态机、admin-only CLI/API 和 transition audit 已在
   Memory Candidate Workflow gate 中闭合；
7. Scoped Memory Production 已把自动策略、人工审核、read enablement、context pack
   memory refs、Runtime projection 可见性、lifecycle、backup/restore 和 public surface
   验证纳入 gate；
8. 完整远端 release automation 已证明 release readiness、saved validator 和 open P0
   retrieval/governance 检查通过。
...
从 `context_pack` 派生、面向单个 consumer 的 Runtime 可见上下文。它是 Context Projection Runtime
...
Context Projection Runtime 只允许 `consumer_type=runtime_profile`，且只支持 `honglou-main`、
`honglou-text`、`honglou-commentary` 和 `honglou-reviewer`。`external_agent`
是保留 consumer 类型，Context Projection Runtime 请求该类型必须 fail-closed。
...
长期 memory 的统一对象。Scoped Context Request Path 和 Context Projection Runtime 不实现 `memory_card`。Memory Candidate Workflow 可以由审核
状态机写出 `memory_card`，但必须保持回答读取面关闭；Scoped Memory Production 才允许
ContextPackBuilder 读取通过 ACL 授权的 active memory 摘要。

最低原则：

1. 必须有 `scope_type + scope_id + profile_name + sharing_policy + ACL`；
2. 默认不能被回答读取；active 状态只能由状态机产生，读取面必须另行打开；
...
15. 审核状态机可以把 approved candidate promote 为 `memory_card`，但 Memory Candidate Workflow
    固定为不可被回答读取；
16. Scoped Memory Production 在 ACL、lifecycle、backup/restore、capacity 和 live gate 闭合后，才允许
    ContextPackBuilder 读取 active memory 摘要；
17. Gateway 返回 OpenAI-compatible response。

Scoped Context Request Path 只实现 1 到 13 中的最小 scoped context 路径，不包含
`context_projection` 强制 Runtime 接入。Context Projection Runtime 实现 9 到 11 的 projection 强隔离。
Memory Candidate Workflow 实现 14 和 15 的候选/状态机闭环，但不能打开读取面。Scoped Memory Production 才实现 16。

## Consumer Projection 可见性

Context Projection Runtime 只支持当前通灵玉 Runtime profile consumer。以下 `honglou-*` 是
`consumer_type=runtime_profile` 下的 `consumer_name`，不是外部用户可指定的模型名。
未知 consumer、未知 Runtime Adapter 和 `external_agent` 在 Context Projection Runtime 必须 fail-closed。
...
4. Memory Collector 允许 LLM 辅助抽取和完整审核状态机；自动 promotion 只能在
   Scoped Memory Production 打开，Memory Candidate Workflow 只允许人工 `promote` 写出不可读 `memory_card`。
...
9. Context Projection Runtime 使用请求级 `context_pack` + consumer 级 `context_projection`。Runtime
...
admin-only CLI/API 审核。Memory Candidate Workflow 细化见
`29_Memory_Candidate_Workflow_Checklist.md`。

Scoped Context Request Path、Context Projection Runtime 和 Memory Candidate Workflow 不做审核页面。Scoped Memory Production 之后若候选量稳定存在，可以在同一套 admin-only
API 之上增加页面，但页面不得绕过状态机。

工作流：

1. Runtime 完成回答后只写 `session_journal`，不直接写 memory；
...
5. 自动 promotion / read enablement 只能在 Scoped Memory Production 打开，并且必须按
   `scoped-memory-policy-v1` 执行；低风险 `user_private` 是默认自动路径，
   `profile_common`、`knowledge_space`、`research_topic` 和 `source_collection`
   按 scope automation matrix 执行有限自动或 manual/shadow；
6. 未命中自动策略、低置信、高风险、scope 不明或 policy 禁止自动的 candidate 必须
   进入审核队列；
7. 未审核 candidate 不参与正式回答；
8. Memory Candidate Workflow 的 `memory_card` 即使为 `active` 状态，也固定不可被回答读取；
9. Scoped Memory Production 才允许 ContextPackBuilder 读取当前 scope 和 ACL 授权的 active memory
   摘要。
...
2. `promote`：在 ACL、scope、retention 和 sensitivity 合法时，从 approved
   candidate 写出 active `memory_card`；Memory Candidate Workflow 固定 `read_enabled=false`；
3. `reject`：拒绝候选并记录原因；
4. `reclassify`：调整 scope、sensitivity 或 candidate type，并记录前后差异；
5. `expire`：让候选过期，不再进入审核队列；
6. `revoke`：撤销已存在的 active `memory_card`；Scoped Memory Production 读取面打开后，后续
   context pack 不得再读取被 revoked 的 memory；
7. `merge`：合并重复候选，保留所有来源 ref。

允许的状态流转分三层：

1. candidate lifecycle：`pending -> approved`；
2. candidate lifecycle：`pending -> rejected`；
3. candidate lifecycle：`pending -> expired`；
4. candidate lifecycle：`pending -> merged`；
5. candidate lifecycle：`pending -> pending`，仅限 `reclassify`，必须写 before/after；
6. card lifecycle：`approved candidate -> active memory_card`；Memory Candidate Workflow 仅限人工
   `promote`，Scoped Memory Production 允许人工 `promote` 或 `scoped-memory-policy-v1` 自动
   `auto_promote`；
7. card lifecycle：`active -> revoked`；
8. card lifecycle：`active -> expired`；
9. read enablement lifecycle：Memory Candidate Workflow 固定 `read_enabled=false`；Scoped Memory Production 才允许在
   scoped memory production gate 下变为可读。
...
Memory Candidate Workflow 中，共享 scope、profile_common、source_collection 或会影响回答策略的
candidate 必须逐条人工审核。Scoped Memory Production 中，这些 candidate 按
`scoped-memory-policy-v1` 的 scope automation matrix 执行：命中有限自动策略时可以
由 policy 自动 approve/promote/enable read；未命中时仍进入 manual review。普通用户
只能查看或撤销自己的 `user_private` memory；admin/operator 可处理队列；reviewer
对影响回答策略的 memory 拥有最终确认权。所有操作都必须写 audit event，禁止通过
手工 SQL 绕过状态机。
...
## Scoped Context Request Path 实现规格

Scoped Context Request Path 只实现 Scoped Context 最小闭环。
...
## Context Projection Runtime 实现规格

Context Projection Runtime 目标是 production-ready Context-aware Runtime，不是本地代码切片完成。
...
`28_Context_Projection_Runtime_Checklist.md`。本文只保留核心规格。
...
5. 未知 consumer、未知 Runtime Adapter 和 `external_agent` 在 Context Projection Runtime fail-closed；
...
## Memory Candidate Workflow 实现规格

Memory Candidate Workflow 实现 Memory Candidate、完整状态机、admin-only CLI/API 和 LLM 辅助抽取。
...
7. `promote` 可以写出 active `memory_card` 状态，但 Memory Candidate Workflow 必须保持读取面关闭；
...
## Scoped Memory Production 实现规格

Scoped Memory Production 的目标不是最小闭环、演示路径或只验证
Collector 通畅，而是让 scoped memory 成为生产链路的一部分：
...
Scoped Memory Production 不重新定义 Memory Candidate Workflow 已验证的候选/卡片状态机；它在
`memory_candidate` / `memory_card` / transition audit 基础上，正式打开受 ACL、
scope、retention、revoke/expire 和 policy 约束的读取面，并把自动策略作为一等生产
路径。人工审核流程必须保留，但不再是所有 memory 进入可用状态的默认必经关卡。

进入条件：

1. Scoped Context Request Path 到 Memory Candidate Workflow 已通过；
2. Memory Candidate Workflow 的 collector、candidate/card 状态机、admin-only CLI/API 和 audit 已在
   hhost production gate 通过；
3. 当前生产结论仍明确不覆盖 active memory 读取、自动 read enablement 或 scoped
   memory production-ready；
4. hhost live gate 未恶化既有 release p95、错误率或降级路径。

冻结原则：

1. 自动策略是允许的生产主路径。memory 可以由规则、LLM 语义过滤和版本化
   policy engine 自动进入可用状态；
2. 人工审核流程必须保留。它用于高风险、低置信、共享 scope、抽样复核、纠错、
   用户撤销、策略回滚和运营排障；
3. 自动路径不得绕过状态机。即使人工审核被配置为跳过，也必须显式记录
   `pending -> approved`、`approved -> active memory_card` 和
   `read_enabled=false -> true` 的 transition audit；
4. LLM 只能做语义过滤和分类辅助，不能直接授权 promotion、ACL、read enablement、
   evidence package、reviewer 裁决或任务状态变化；
5. policy engine 是唯一决策者。它读取规则结果、LLM 结构化输出、scope、sensitivity、
   confidence、risk flags、retention 和 ACL，决定 `suppress`、`pending_manual_review`、
   `auto_approve`、`auto_promote`、`enable_read` 或 `disable_read`；
6. Scoped Memory Production 默认 policy version 固定为 `scoped-memory-policy-v1`。policy mode 默认
   为 `auto_policy`，并保留 `shadow_only` 和 `manual_required` 降级；阈值、TTL、
   scope automation matrix、LLM schema、candidate type allowlist 和 read budget
   均由该 policy version 定义，生产变更必须形成新 policy version；
7. scoped memory 不是 `user_private` 的降级实现。Scoped Memory Production 必须保留
   `user_private`、`profile_common`、`knowledge_space`、`research_topic` 和
   `source_collection` 的 ACL、读取策略和 gate。不同 scope 可以有不同自动化门槛，
   但都必须可被同一套 policy/audit/context build 处理；
8. memory 只能作为偏好、背景、工作方法、长期上下文摘要进入受控 context。它不能
   作为正式事实源，不能进入 evidence package，不能替代原文/脂批/版本证据，不能
   改变 reviewer 裁决；
9. Gateway 仍不能变成长期 memory 系统。Gateway 可以在当前实现阶段同进程执行
   Context Governance，但 memory 的生产语义必须由独立的 policy、ACL、audit 和
   lifecycle 边界表达。

允许实现：

1. `memory_card` 的 read enablement lifecycle、ACL enforcement 和读取策略；
2. 自动 promotion / read enablement policy，包括低风险偏好、稳定背景和工作方法类
   memory 的自动可用路径；
3. `profile_common`、`knowledge_space`、`research_topic`、`source_collection` 或
   共享 scope memory 的 policy 约束、人工复核和受控读取路径；
4. ContextPackBuilder 读取当前 scope 和 ACL 授权的 active/read-enabled memory
   摘要，并写入 `memory_read_refs`；
5. Runtime projection 按 consumer/profile 只接收授权 memory 摘要；
6. export、anonymize、legal hold 覆盖 memory candidate、policy decision、
   memory card、read enablement 和相关 audit；
7. backup/restore 验证覆盖 context、journal、memory、package、review 和 audit 链；
8. 可选审核页面，但只能调用同一套 admin-only API，不能绕过状态机和 audit。

自动策略流程：

1. Runtime 完成回答后写 `session_journal`，不直接写 memory；
2. Memory Collector 扫描已完成 trace/context 的 journal；
3. 规则 hard deny 先执行，过滤 secret、token、系统提示词、prompt injection、
   source fact、reviewer 裁决、签署状态、任务关闭状态、action result、临时指令和
   明确测试输入；
4. LLM semantic filter 只接收 redacted 输入，输出结构化 JSON，用于判断长期偏好、
   临时要求、讽刺/引用、矛盾、scope、sensitivity、TTL 建议、confidence 和
   risk flags；
5. policy engine 做最终裁决；
6. 命中自动策略时，由 actor=`memory_policy:auto:scoped-memory-policy-v1` 写完整
   transition audit；
7. 未命中自动策略但仍有价值的 candidate 保留为 `pending_manual_review`；
8. ContextPackBuilder 仅读取 `status=active`、`read_enabled=true`、未 revoked、
   未 expired、ACL 匹配当前 scope 的 memory 摘要；
9. revoke、expire、legal hold、anonymize 或 policy disable 后，新 context pack 必须
   立即停止读取相关 memory。

读取边界：

1. `honglou-main` 可以读取当前用户/当前 scope 授权的 memory 摘要，用于偏好、背景和
   工作方法；
2. `honglou-text` 和 `honglou-commentary` 只能读取与检索策略明确相关且授权的
   非隐私摘要，默认不得读取 `user_private`；
3. `honglou-reviewer` 不能把 memory 当证据，也不能因为 memory 改变 reviewer 裁决；
4. evidence package、claim link、source citation 和 reviewer enforcement 均不得引用
   memory 作为事实依据；
5. public response、SSE、metrics 和普通日志不得暴露 memory card id、candidate id、
   policy payload、journal 原文或高基数 internal ref。
6. ContextPackBuilder 必须执行 read budget。默认每个 context pack 最多 8 条 memory
   ref，其中 `user_private` 最多 4 条，shared scope 总计最多 4 条；超出预算时按
   confidence、recency、scope specificity 和 policy priority 排序截断，并写 read
   decision audit。
...
5. 自动策略和人工审核都走同一套状态机、service、audit 和 lifecycle；
6. LLM 输出越权字段、非法 scope、低置信或 hard deny 命中时 fail-closed；
7. revoke/expire/disable 后新 context pack 不再读取相关 memory；
8. export、anonymize、legal hold 覆盖 context、journal、candidate、policy decision、
   memory card 和 read enablement audit；
9. backup/restore 后 context/journal/package/reviewer/memory 链可恢复；
10. live release report 证明 capacity、error rate、privacy、restore 和 post-release
    monitor 通过。
...
1. Scoped Context Request Path 和 Context Projection Runtime 退出条件全部通过；
...
1. Memory Candidate Workflow 和 Scoped Memory Production 退出条件全部通过；
...
3. Context Projection Runtime 已实现并通过当次 `hhost` production gate；
4. Memory Candidate Workflow 已实现并通过当次 `hhost` production gate；
5. `session_journal`、Memory Collector、`memory_candidate`、`memory_card`、
   状态机、admin-only CLI/API 和 transition audit 已进入生产候选/审核路径；
6. Scoped Memory Production 已打开受 `scoped-memory-policy-v1`、ACL、scope、TTL、revoke/expire、
   lifecycle 和 projection 约束的 active memory 读取路径；
7. 自动策略可以让符合 policy 的 memory 显式完成
   `pending -> approved -> active memory_card -> read_enabled=true`，人工审核路径保留
   并使用同一状态机和 audit；
8. ContextPackBuilder 会写入 `memory_read_refs`、`memory_read_ref_digest` 和
   `memory_read_policy_digest`，Runtime projection 只接收授权 consumer 可见的摘要；
9. policy engine、manual promote 和 read path 都要求来源是已绑定 `context_pack_id`
