...
- [ ] 新增并冻结本文 checklist。
- [ ] 明确 code invariant 与外部语义规则边界：
  - [ ] code invariant：幂等 key、状态机、source hash、schema version、promote
        upsert、trace event。
  - [ ] 外部规则：slot、entity role、polarity、modality、evidence strength、
        source scope、哪些材料类型能支持哪些 claim。
- [ ] 明确 `raw_evidence_candidate`、`canonical_staged_card`、
      `evidence_card` 三层模型。
- [ ] 明确 query expansion 只负责找材料，不表达 slot 语义、合并语义或冲突语义。
- [ ] 新增反硬编码检查清单：主流程不得含具体问题、人物、章节或 eval case 分支。
- [ ] 明确 promoted 之前必须先实现合并和冲突阻断；不能先做 happy path promote，
      再把冲突处理留到后续补丁。
...
- [ ] coverage checker 在 package 覆盖不足时创建
      `staged_card_update_request`，不阻塞请求上下文。
- [ ] background worker 消费 staging queue，执行 local retrieval、card builder、
      validator 和自动 promote。
- [ ] staged card、raw candidate、events 都可通过 admin / trace 查询。
- [ ] 后续请求能查询 pending staged card，用于去重和观察，但不能把 pending card
      放入最终 package。
- [ ] 最终回答只读取已 promoted 的 evidence card。
- [ ] worker 完成 promote 后，后续请求从 card store 自然受益。
- [ ] trace 记录 coverage gap、update request、raw candidate、staged card、
      merge/conflict、validator、promote 结果。
- [ ] LLM recall advisor 只写 recall advice，不写 card、不决定 promote。
- [ ] rules 不足时生成 `raw_evidence_candidate` 和 `rule_gap` event，不 promote。
- [ ] 实现 `exact_span_key` 幂等合并。
- [ ] 实现 `claim_key` 合并。
- [ ] 实现 span 重叠且 claim 相同的 canonical span 选择。
- [ ] 实现 promoted card 覆盖同一 claim 时的 `superseded_by_promoted`。
- [ ] 实现同一 span 多 slot 的分卡逻辑。
- [ ] 实现相似但不确定同 claim 的 cluster-only 逻辑。
- [ ] 实现 polarity、role、scope、modality、evidence strength、entity resolution、
      source hash、schema、rule、builder version 冲突阻断。
- [ ] 实现冲突 event 查询和 trace 展示。
- [ ] Rust module tests 放入单独 tests 文件，覆盖 staging request、worker 主路径、
      staged 可查询、package 禁用 staged card、promote 后可读。
- [ ] 补 property-style 或 table-driven tests，证明合并不能提高证据强度。
...
- [ ] 新增 durable `card_ingest_job`，关联 update request、candidate、staged card。
- [ ] 实现 `leased_by`、`lease_until`、heartbeat、`attempt_count`、`last_error`。
- [ ] 实现启动 reconciler，恢复 `queued`、`processing` 超时、`promote_failed`、
      lease expired 的 job。
- [ ] 实现 retry policy、backoff 和 dead letter。
- [ ] job event stream 可关联原始 trace，后续异步事件不丢失。
- [ ] worker crash / restart 后从最近安全阶段继续，不重跑整条用户请求。
- [ ] promote/upsert 全程幂等，重复执行不制造重复 card。

### Phase 3: Package、answer、reviewer 与 eval gate

- [ ] package builder 只读取 promoted evidence card。
- [ ] answer composer 对 coverage partial 做业务化表达，不泄露 staged、job、trace
      等内部字段。
- [ ] reviewer 拒绝使用 staged / raw candidate / recall advice 作为正式证据。
- [ ] admin trace 显示 staged/promotion 链路，普通响应不泄露内部治理字段。
- [ ] 既有 eval suite 和代表性多轮领域对话回归通过，但不得把回归 case 写成
      主流程特例。
- [ ] 新增在线补卡 fixture：第一次请求创建 staging request，worker promote 后，
      第二次请求使用 promoted card。
- [ ] 新增规则缺口 fixture：保存 raw candidate，但不进入 package。
- [ ] 新增冲突 fixture：相反 polarity、不同 source scope、不同 modality 均阻断
      promote。
- [ ] 新增反硬编码 fixture：更换实体名、slot alias、source scope 后，流程仍按
      contract 工作，不依赖固定问题文本。
- [ ] hhost 部署后验证版本、运行镜像、OCI label、远端 env、worker health、
...
- [ ] checklist 文档通过 `markdownlint-cli2` 和 `git diff --check`。
- [ ] schema / rules 边界有独立 review 记录：哪些是 code invariant，哪些是
      external rules，哪些禁止进入 query expansion。
- [ ] 反硬编码检查有可运行命令或脚本，至少扫描主流程代码中的具体问题文本、
      人物名、章节名、eval id 和临时 `if/else`。
- [ ] 文档地图已接入本 checklist，后续实现不得绕过本文阶段边界。
- [ ] Phase 0 关闭时必须有提交记录；不能把未冻结 checklist 当作实现依据。

### Phase 1 验收

- [ ] Rust unit tests 覆盖 `staged_card_update_request` 创建、raw candidate 写入、
      canonical staged card 生成、validator、promote 和 package reload。
- [ ] Rust unit tests 覆盖 package builder 拒绝 staged / raw candidate / recall
      advice 进入最终 package。
- [ ] table-driven tests 覆盖 exact span merge、claim merge、overlap merge、
      cluster-only、multi-slot split、superseded-by-promoted。
- [ ] table-driven tests 覆盖 polarity、role、scope、modality、strength、
      entity resolution、source hash、schema / rule / builder version 冲突。
- [ ] 集成测试覆盖一次请求创建 staging request，worker 自动处理，后续请求读取
      promoted card；当前请求不使用 pending card。
- [ ] trace fixture 覆盖 coverage gap、update request、raw candidate、merge/conflict、
      validator、promote event。
- [ ] 失败验收覆盖 worker / validator / promote 失败时不写入 package、不伪装成功。

### Phase 2 验收

- [ ] Rust unit tests 覆盖 lease acquire、heartbeat、lease expired 接管、attempt
      count、last error 和 retry backoff。
- [ ] Rust unit tests 覆盖 reconciler 从 `queued`、`processing` 超时、
      `promote_failed`、dead letter 前状态恢复。
- [ ] fault-injection integration test 覆盖 worker 中断后重启，从最近安全阶段继续。
- [ ] 幂等回放测试覆盖重复执行同一 job 不产生重复 card、不重复 promotion event。
- [ ] job event stream 测试覆盖原始 trace 与后续异步 event 可关联。
- [ ] 运维验收覆盖 worker health、queue depth、retry count、dead letter count 指标。

### Phase 3 验收

- [ ] package / answer / reviewer 单元测试覆盖：只读取 promoted card，拒绝 staged、
      raw candidate、recall advice 作为正式证据。
- [ ] public response fixture 覆盖 coverage partial 的业务化表达，不泄露 staged、
      job、trace、rule id 或内部状态机。
- [ ] eval suite 覆盖在线补卡、规则缺口、冲突阻断、反硬编码和代表性多轮领域对话。
- [ ] hhost deploy gate 覆盖运行镜像版本、OCI label、远端 env、worker health、
      admin trace、eval report。
- [ ] 回归结果必须同时包含成功路径和受控失败路径；不能只用 happy path 证明上线。

## Exit Criteria

- [ ] 所有新增 Rust module tests 独立放在 tests 文件中。
...
- [ ] `cargo test --manifest-path agent-platform/Cargo.toml -p tonglingyu-gateway`
      通过。
- [ ] `scripts/qa.sh --quick` 通过。
- [ ] hhost 部署后能证明：
  - [ ] 运行镜像版本等于 source `VERSION`；
  - [ ] OCI label 等于 source `VERSION`；
  - [ ] worker 正常消费 staging queue；
  - [ ] trace 能看到 online ingest 链路；
  - [ ] final package 不包含未 promoted card；
  - [ ] eval report 通过。
