...
- `cargo clippy --manifest-path agent-platform/Cargo.toml -p tonglingyu-gateway
  --all-targets -- -D warnings`
- `bash -n ../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-strict-gateway.sh`
...
cargo clippy \
  ...
  --all-targets -- -D warnings
bash ../tonglingyu-gatekeeper/deploy/scripts/test-openwebui-gateway-admin-action-contract.sh
```
...
cargo clippy \
  ...
  -p tonglingyu-gateway \
  --all-targets -- -D warnings
bash ../tonglingyu-gatekeeper/deploy/scripts/test-tonglingyu-release-readiness-contract.sh
```
...
8. `../tonglingyu-gatekeeper/deploy/scripts/test-openwebui-gateway-admin-action-contract.sh`
9. `../tonglingyu-gatekeeper/deploy/scripts/test-tonglingyu-release-readiness-contract.sh`
10. `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-knowledge-state-release.sh`
11. `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness.sh`
12. `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness-report.sh`
...
2. `agent-platform/scripts/tonglingyu-calibration-job-smoke.sh`：固定运行
   ...
   fail-closed 覆盖。
3. `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-knowledge-state-release.sh`：串联 release readiness
