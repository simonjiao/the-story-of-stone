...
这些改动仍不能宣布生产完成：`agent-runtime`/Hermes 已进入 profile
...
- [x] browser review evidence verifier 会输出 evidence JSON SHA-256 和本地
  ...
  `public_webui_url`，避免生产发布记录只保存可变文件路径、却无法从 release
  report 本身判断谁在何时复核了哪个公网入口。
- [x] saved report validator 会重新校验 `browser_review_validation` 中的
  ...
  `browser_review_evidence` 必须保留，四项都必须有 ref，ref kind 必须在允许集合内，
  本地文件 ref 必须保留 SHA-256，且 `browser_review_evidence` 指向的 evidence
  JSON 必须与 `browser_review_validation.evidence_sha256` 匹配；本地文件 ref
  也会按 evidence 目录或 `TONGLINGYU_BROWSER_REVIEW_EVIDENCE_ROOT` 重新计算
  digest，避免 release artifact 塞入未定义检查项、空数组、假 digest 或
  不可复核的 evidence ref 摘要。
...
- [ ] 目标环境 Open WebUI browser-side 单入口复测与人工 ACK。
