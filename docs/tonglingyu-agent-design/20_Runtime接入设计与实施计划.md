...
- [x] saved report validator 会重新校验 `browser_review_validation` 中的
  ...
  exact required set，四项都必须有 ref，ref kind 必须在允许集合内，本地文件
  ref 必须保留 SHA-256，且 `browser_review_evidence` 指向的 evidence JSON
  必须与 `browser_review_validation.evidence_sha256` 匹配，避免 release
  artifact 塞入未定义检查项、空数组、假 digest 或不可复核的 evidence ref 摘要。
