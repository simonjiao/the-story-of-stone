...
- [x] 增加 `deploy/scripts/verify-tonglingyu-release-readiness.sh` 聚合 gate，
  ...
  `browser_review_acknowledged`、skipped live gate、release blockers 和人工页面
  复核项；只有显式 `TONGLINGYU_RELEASE_REQUIRE_LIVE=true` 且 live gate 与页面
  ACK 都满足时才会标记 production ready；默认退出码跟随
  `production_release_ready`，只有显式 `TONGLINGYU_RELEASE_SUMMARY_ONLY=true`
  才允许生成非发布 summary 时返回成功。
- [x] release gate 在 `TONGLINGYU_RELEASE_REQUIRE_LIVE=true` 时要求显式
  `TONGLINGYU_RELEASE_ACK_OPENWEBUI_BROWSER_REVIEW=true` 和非空
  `TONGLINGYU_RELEASE_OPENWEBUI_BROWSER_REVIEW_REF`，避免自动 gate 通过但
  Open WebUI 页面侧未复测、或复测没有可追溯证据时仍显示生产发布通过。
