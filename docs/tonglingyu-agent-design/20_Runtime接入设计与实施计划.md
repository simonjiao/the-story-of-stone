...
- [ ] 远程 `hhost` strict Gateway live gate 通过。当前 blocker 是 Hermes
  后端模型链路经 `sub2api` 请求 `gpt-5.4-mini` 返回 502/超时；新请求会
  fail-closed 并留下 `agent_runtime_profile_execution_rejected` admin trace。
