...
- [x] 增加 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-runtime-config.sh`，基于 compose
...
- [x] 生产 compose 显式设置 Gateway `TONGLINGYU_AGENT_RUNTIME_MODE=hermes`
  ...
  仍为 `minimal` 的渲染结果。
- [x] 增加 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-strict-gateway.sh`，运行态检查
...
- [x] 增加 `../tonglingyu-gatekeeper/deploy/scripts/verify-tonglingyu-release-readiness.sh` 聚合 gate，
