# 多机器多 Agent 校验知识库服务方案

本服务尚未实现。本文件记录已确定的实现方向，供后续开发直接执行。

## 目标

构建 Docker 化的《红楼梦》校验知识库服务，支持多机器、多 agent 共同访问同一套基础资料、研究资料、风格资料和校订记录。该服务是后续交互机器人的基础设施。
...
- `editions`: 《红楼梦》发行版本信息，v1 先记录当前 base corpus，后续扩展多个版本。
- `chapters`: 回目、标题、章节序号。
- `blocks`: 文本块、类型、章节、段落号、来源路径。
- `blocks_fts`: FTS5 精确检索索引。
- `terms`: 术语、出现章节、例句、规则说明。
- `glyph_images`: 图片字形路径、所在章节、前后文、可选确认字符。
- `block_embeddings`: `block_id`、embedding、模型名、维度、生成版本。
- `styles`: 风格档案，记录风格名、来源语料、适用场景和表达约束。
- `kb_version`: base corpus 来源、生成时间、代码 commit、embedding 模型。

资料分类规则：

- `base_material`: 当前《红楼梦》原文、注释/校记、图片字形和元数据，可直接引用和用于校订。
...
- 向量结果只用于候选召回。
- 最终校订建议必须落到 SQLite 中的原文、注释、批语、术语或图片字形证据。
