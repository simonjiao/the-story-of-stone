...
1. Runtime streaming 输出：R2 已补齐 `started` / `delta` / `final` /
   ...
   语义；无工具 streaming 路径解析 Hermes 上游 SSE，tool loop streaming
   路径返回合成后的有序 Runtime events；streaming 路径失败时返回安全
   `error` event，不把 upstream response、prompt 或内部错误细节写入 event。
...
4. schema validation 只返回安全错误类型，不回显输入侧未知字段名/值、
   schema required 字段名、schema property path 或 raw value。
...
1. `agent-core`：新增 `RuntimeStreamEvent` 和 event-returning streaming trait
   边界。
2. `agent-runtime`：为 Hermes adapter 增加无工具上游 SSE streaming path；
   tool loop streaming path 复用受控 tool loop 并合成有序 Runtime events。
...
3. 每个 tool call 在执行前校验 tool name、arguments JSON、input schema 和
   profile tool policy。
...
2. 未授权、denied、无 profile contract 或 no-tool run/session path 中的
