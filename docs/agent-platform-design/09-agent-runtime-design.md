...
6. Runtime tool execution loop：R4.5 已补齐 read-only tool call、
   tool input/output schema、预算、metadata 摘要、成功/失败 tool audit、
   Worker 路径审计和 Runtime adapter 直连 JSONL audit sink；具体领域
   Gateway 仍需显式配置并复测。
...
5. `agent-core` / `agent-runtime`：streaming path 失败时返回安全 `error`
...
8. tool call 失败时必须写安全 `runtime_tool_error` audit event，且不记录
