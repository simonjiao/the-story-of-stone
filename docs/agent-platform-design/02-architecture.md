...
Agent Manager admin API
  ↓
审批 / 授权 / 暂停 / 恢复 / 审计 / Observer report 查询
```
...
| agentctl CLI | 管理员入口 | 审批、查询、暂停、恢复、审计、Observer report 查询 | 不面向普通用户 |
...
4. agent-runtime、agent-worker、agent-observer、agentctl 可以访问 agent-manager。
...
Worker → Manager internal run API
