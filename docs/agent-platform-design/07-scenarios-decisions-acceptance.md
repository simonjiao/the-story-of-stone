...
2. Manager 是唯一控制面；授权、审批、生命周期、资源锁和审计不下放给 Runtime / Worker / Observer。
3. P0 正确性边界是 Postgres lease / SKIP LOCKED；Redis 只作为后续 adapter。
4. Observer 只读，只生成 observer_report；report discussion 由普通 agent_session 承载。
5. P1 接真实 Hermes Runtime 但只读；P2 只启用 P1 已固定的真实 provider / connector adapter。
6. 后续新增 transport、memory、connector、queue、telemetry 实现时，不重写 domain model、API contract、状态机或 audit contract。
```
...
| S16 P1 Hermes Runtime | RuntimeClient 接真实 Hermes，但不持有写 credential | 01, 08 |
| S17 P2 外部执行 | 经过审批、最小权限 credential、resource lock 和审计 | 01, 04, 05, 08 |
| S18 Observer 定期评测 | 读取只读摘要，生成 `observer_report` | 02, 03, 04, 05 |
| S19 Observer 尝试控制动作 | 硬拒绝并写 audit | 01, 04, 06 |
| S20 管理员查询 Observer report | 通过 agentctl/admin API 查询，Open WebUI 不可见 | 02, 03, 05 |
| S21 高风险副作用 | 默认审批或硬拒绝，不能绕过 Manager | 01, 06, 08 |
| S22 Agent paused / failed / terminated | 按状态动作矩阵返回安全摘要或管理员操作 | 04 |
| S23 并发 run 争用同一资源 | Postgres lease + resource_locks 保证同一资源副作用串行 | 05 |
| S24 新增 adapter | 只新增 trait 实现、adapter 或 feature，不改核心 contract | 05, 08 |
| S25 Observer Report Discussion | Manager 创建普通 session 讨论 report；Observer 不参与对话、不执行动作 | 03, 04, 05, 08 |
| S26 P1 P2 readiness dry-run | P1 固定 side-effect / credential / write connector contract，只 dry-run；P2 启用真实 adapter | 04, 05, 08 |
...
6. Worker 可以通过 Postgres lease claim、heartbeat、timeout 和 dead_letter 管理 run。
7. 每次 run 可追溯到 request、user、service、session、resource、result 和 audit。
8. P0 已包含 telemetry facade 和核心 trait 边界。
```
...
3. Observer 可以评测 Runtime 延迟、失败、重试和上下文膨胀。
4. 授权 operator 可以围绕 observer_report 创建受控 discussion session。
5. P1 已提供 side-effect plan、credential lease、write connector contract、dry-run policy、no-op provider 和审计事件。
6. P1 不改 request、run、session、audit、Manager 授权或 Worker claim 基线模型。
```
