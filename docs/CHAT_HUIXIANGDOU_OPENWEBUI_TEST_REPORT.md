...
| [ISSUE-005](chat_huixiangdou_issues/ISSUE-005-followup-prompt-p0-false-positive.md) | P1 | Open WebUI 自动追问建议提示包含历史中的 P0 指令时，被 Orchestrator 误判为新的 P0 请求。 | P0 请求列表和审计被内部辅助提示污染，可能产生重复审批噪音。 | RESOLVED: 已收窄识别范围并部署复测 |
...
| [ISSUE-008](chat_huixiangdou_issues/ISSUE-008-open-webui-p0-identity-session-bridge.md) | P2 | Open WebUI 默认调用未透传动态 P0 用户身份和 agent session 元数据。 | 多用户审计和 UI 级 P0 session 绑定不能依赖当前默认聊天请求。 | OPEN: 后端 API 已支持多 session；UI 桥接需插件、代理或 JWT 注入 |
...
| 2026-05-08 18:20 CST | 使用测试账号重新执行 UI 关键路径与长回答专项。 | 登录成功，模型选择器显示 `hermes-agent`；UI 普通聊天返回 `ROUTE_UI_OK`；UI P0 指令返回 `approval_required`。 |
...
| 2026-05-08 18:33 CST | 发现并修复 Open WebUI 追问建议导致的 P0 误判。 | 追问建议内部提示曾额外创建 `req_019e071a9a667a13a10be4f718ee3746`；修复 Orchestrator 只识别 `### Chat History` 之前的直接用户文本。 |
| 2026-05-08 18:38 CST | 部署并复测 P0 误判修复。 | 内部追问建议提示复测前后最新请求 ID 不变，响应不含 `approval_required`；直接用户控制指令仍创建 `req_019e072a5bc87352b4ca99c26664157f`。 |
...
| 2026-05-08 18:58 CST | 执行 P0 多 session 与 Worker run 复测。 | 新请求 `req_019e073c5df57450902773f4c872c1d0` 审批后 agent 归属原始请求人；同一 agent 下两个 session、两个 run 均完成。 |
...
| P0-UI-CTRL-20260508 | PASS | UI 控制类请求进入 P0 Manager 并要求审批。 | UI 回复包含 `request_id=req_019e071a9a2572c08488577cc52d77d6`、`status=approval_required`。 |  |  |
| P0-UI-AUDIT-20260508 | PASS | 远程 Manager 可见 UI 触发的 P0 请求。 | `agentctl requests list` 包含 `req_019e071a9a2572c08488577cc52d77d6`，状态 `approval_required`。 |  |  |
...
| P0-FOLLOWUP-FP-20260508 | FAIL->PASS | Open WebUI 追问建议内部提示曾误创建 P0 请求；修复部署后不再复现。 | 修复前额外请求 `req_019e071a9a667a13a10be4f718ee3746`；修复后同类提示复测前后最新请求 ID 不变，且响应不含 `approval_required`。 | P1 | 见 `ISSUE-005`。 |

## 2026-05-08 P0 Owner、多 Session 与 Worker 复测

| 用例 ID | 状态 | 实际结果 | 证据 | 问题等级 | 备注 |
| --- | --- | --- | --- | --- | --- |
| P0-APPROVAL-OWNER-20260508 | FAIL->PASS | 审批后 agent 归属原始请求人，不再归属审批人。 | 请求 `req_019e073c5df57450902773f4c872c1d0` 审批后生成 `agent_019e073c5ea27640b34fc51a49be5f96`；`codex-owner-20260508` 的 `my-agents` 命中 1 条，`admin` 的 `my-agents` 命中 0 条。 | P1 | 见 `ISSUE-007`。 |
| P0-MULTI-SESSION-20260508 | PASS | 同一 agent 下可创建并保持两个活跃 session。 | `sess_019e073cf05b72f08e3d20c625c86443`、`sess_019e073cf1eb724188643a0e15effef2` 均为 `active`；`agentctl agents list` 显示 `active_session_count=2`。 |  | API 路径验证；UI 默认聊天桥接见 `ISSUE-008`。 |
| P0-WORKER-RUN-20260508 | PASS | Worker 能处理同一 agent 的两个 session run。 | `run_019e073cf1637c40adb597c8960649ed`、`run_019e073cf2f87580add2d1e6500811af` 均为 `completed`，摘要均包含 `with 1 recent messages`。 |  | 审计包含 `worker:run_claim`、`worker:run_status`、`worker:run_finish`。 |
| P0-OBSERVER-20260508 | PASS | Observer 在 Worker run 后继续报告健康。 | 最新报告 `obsr_019e073d65d67222ac9e55bd2822a7e5` 为 `healthy`，摘要 `dead_letter=0`。 |  |  |
...
5. `ISSUE-005` 已在 Orchestrator 中修复并部署，Open WebUI 追问建议不会再污染 P0 请求列表。
6. `ISSUE-006` 不阻断核心路径；建议后续补充停止生成按钮可访问标签。
7. `ISSUE-007` 已在 Manager 中修复并部署，审批后的 agent 归属原始请求人，Worker 复用和多 session API 路径通过。
8. `ISSUE-008` 仍开放：Open WebUI 默认聊天没有动态 P0 身份/session 桥接；若需要多用户 UI 级 P0 session，需要专门桥接实现。
