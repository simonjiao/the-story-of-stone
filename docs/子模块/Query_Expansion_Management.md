...
`query_expansions.json` 只声明“哪些词可以把材料找出来”。运行时不再维护单独的 evidence slot rules，也不再用预置 slot 语义判断某条证据能否计数、能否支持人物结局、或能否进入公开回答。只要 retriever 返回了可追溯证据，draft/review/final 阶段就应围绕证据本身综合判断。
...
5. 不要在 query expansion 中重新引入 slot、计数口径或人物结局 oracle；这些判断只能来自 retriever evidence 和后续 evidence-based draft/review。
