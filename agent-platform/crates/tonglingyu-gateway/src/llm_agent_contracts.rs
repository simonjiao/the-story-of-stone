...
pub(crate) struct QuestionNormalizerAgentInput {
    ...
    pub(crate) allowed_rule_candidate_types: Vec<String>,
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            allowed_context_refs: LLM_RESOLVER_ALLOWED_CONTEXT_REFS
                ...
                .collect(),
            allowed_referents,
            allowed_rule_candidate_types: RULE_CANDIDATE_TYPES
                .iter()
                .map(|item| (*item).to_string())
                .collect(),
...
pub(crate) struct RuleCandidateSuggestion {
...
pub(crate) struct QuestionNormalizerAgentOutput {
    ...
    #[serde(default)]
    pub(crate) question_frame_candidate: Option<Value>,
    #[serde(default)]
    pub(crate) rule_candidates: Vec<RuleCandidateSuggestion>,
}
...
fn read_only_profile_contract(profile_id: &str, version: &str, timeout_ms: u64) -> ProfileContract {
    ...
    contract.max_runtime_seconds = Some(timeout_ms.div_ceil(1_000).max(1));
