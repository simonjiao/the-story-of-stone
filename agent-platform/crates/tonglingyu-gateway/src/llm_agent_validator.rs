...
pub(crate) struct SealedQuestionResolution {
    ...
    question_frame: Option<QuestionFrame>,
}
...
impl SealedQuestionResolution {
    ...
    pub(crate) fn question_frame(&self) -> Option<&QuestionFrame> {
        self.question_frame.as_ref()
    }
}
...
impl ProviderAuditSnapshot {
    fn from_runtime_metadata(runtime_metadata: Option<&Value>) -> Self {
        Self {
            ...
            latency_ms: provider_latency_ms(runtime_metadata),
        }
    }
...
pub(crate) fn validate_question_normalizer_runtime_output(
    ...
    let candidate_frame_required =
        !parsed_output.needs_clarification && parsed_output.confidence >= 0.75;
    let mut question_frame_candidate_errors = Vec::new();
    let question_frame = parsed_output
        .question_frame_candidate
        .as_ref()
        .and_then(|candidate| {
            match validate_agent_question_frame_candidate(
                candidate,
                &parsed_output.resolved_question,
            ) {
                Ok(frame) => Some(frame),
                Err(error) => {
                    if candidate_frame_required {
                        errors.push(error.to_string());
                    } else {
                        question_frame_candidate_errors.push(error.to_string());
                    }
                    None
                }
            }
        });
    validate_rule_candidates(&parsed_output.rule_candidates, &mut errors);
    ...
    } else if parsed_output.needs_clarification || parsed_output.confidence >= 0.45 {
    ...
    let rule_candidates = parsed_output.rule_candidates.clone();
    let question_frame_candidate_present = parsed_output.question_frame_candidate.is_some();
    let question_frame_candidate_accepted = question_frame.is_some();
    ...
    QuestionNormalizerValidationDecision {
        audit: question_audit(
            ...
            schema_repaired_locally,
            &rule_candidates,
            question_frame_candidate_present,
            question_frame_candidate_accepted,
            &question_frame_candidate_errors,
            &provider_audit,
        ),
        accepted,
        errors,
...
fn validate_rule_candidates(candidates: &[RuleCandidateSuggestion], errors: &mut Vec<String>) {
    if candidates.len() > 8 {
        errors.push("rule_candidates_too_many_items".to_string());
    }
    for candidate in candidates {
        if !RULE_CANDIDATE_TYPES.contains(&candidate.candidate_type.as_str()) {
            errors.push("rule_candidate_type_not_allowed".to_string());
        }
        if candidate.term.trim().is_empty() || candidate.term.chars().count() > 80 {
            errors.push("rule_candidate_term_invalid".to_string());
        }
        if candidate.reason.trim().is_empty() || candidate.reason.chars().count() > 240 {
            errors.push("rule_candidate_reason_invalid".to_string());
        }
    }
}
...
fn question_audit(
    ...
    rule_candidates: &[RuleCandidateSuggestion],
    question_frame_candidate_present: bool,
    question_frame_candidate_accepted: bool,
    question_frame_candidate_errors: &[String],
    provider_audit: &ProviderAuditSnapshot,
) -> Value {
    json!({
        ...
        "errors": errors,
        "provider_latency_ms": provider_audit.latency_ms.clone(),
        "provider_output_features": provider_audit.output_features.clone(),
        "provider_request": provider_audit.request.clone(),
        "rule_candidate_count": if contract_accepted { rule_candidates.len() } else { 0 },
        "rule_candidates": if contract_accepted {
            json!(rule_candidates)
        } else {
            json!([])
        },
        "question_frame_candidate_present": question_frame_candidate_present,
        "question_frame_candidate_accepted": question_frame_candidate_accepted,
        "question_frame_candidate_errors": question_frame_candidate_errors,
...
fn provider_latency_ms(runtime_metadata: Option<&Value>) -> Value {
    runtime_metadata
...
#[cfg(test)]
#[path = "llm_agent_validator/tests.rs"]
mod candidate_tests;
