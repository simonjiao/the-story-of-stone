use crate::{
    EVIDENCE_SLOT_RULES_PATH_ENV, normalize_text,
...
static EVIDENCE_SLOT_RULES_CATALOG_CACHE: OnceLock<Mutex<RuleFileCache<EvidenceSlotRuleCatalog>>> =
    OnceLock::new();
...
fn evidence_slot_rule_catalog() -> Result<EvidenceSlotRuleCatalog> {
    let path = configured_path(EVIDENCE_SLOT_RULES_PATH_ENV);
    let cache = EVIDENCE_SLOT_RULES_CATALOG_CACHE
        .get_or_init(|| Mutex::new(default_evidence_slot_rule_cache()));
    let mut cache = lock_rule_cache(cache, "evidence slot")?;
    cache.catalog(
        EVIDENCE_SLOT_RULES_PATH_ENV,
        path,
        default_evidence_slot_rule_catalog(),
        parse_evidence_slot_rule_catalog,
    )
}

pub(crate) fn evidence_slot_rule_catalog_metadata() -> Result<Value> {
    let path = configured_path(EVIDENCE_SLOT_RULES_PATH_ENV);
    let cache = EVIDENCE_SLOT_RULES_CATALOG_CACHE
        .get_or_init(|| Mutex::new(default_evidence_slot_rule_cache()));
    let mut cache = lock_rule_cache(cache, "evidence slot")?;
    ...
    Ok(cache.metadata(EVIDENCE_SLOT_RULES_SCHEMA_VERSION, &catalog.catalog_version))
}

fn default_evidence_slot_rule_cache() -> RuleFileCache<EvidenceSlotRuleCatalog> {
    RuleFileCache::new(default_evidence_slot_rule_catalog())
}

fn default_evidence_slot_rule_catalog() -> EvidenceSlotRuleCatalog {
    parse_evidence_slot_rule_catalog(DEFAULT_EVIDENCE_SLOT_RULES_JSON)
...
fn parse_evidence_slot_rule_catalog(source: &str) -> Result<EvidenceSlotRuleCatalog> {
    ...
    for slot in &catalog.slots {
        ...
        for subject in &slot.supports_subjects {
            if subject.trim().is_empty() {
                return Err(anyhow!(
                    "evidence slot rule {} has an empty supports_subjects value",
                    slot.id
                ));
            }
        }
        ...
        {
            return Err(anyhow!(
                "evidence slot rule {} support_modality must be non-empty when provided",
                slot.id
            ));
        }
        if slot
        ...
        {
            return Err(anyhow!(
                "evidence slot rule {} evidence_strength must be non-empty when provided",
                slot.id
            ));
        }
    }
...
pub(crate) fn validate_evidence_slot_rule_catalog_source(source: &str) -> Result<()> {
    parse_evidence_slot_rule_catalog(source).map(|_| ())
}
