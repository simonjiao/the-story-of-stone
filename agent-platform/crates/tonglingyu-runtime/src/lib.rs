...
pub use online_evidence_card_ingest::{
    CanonicalStagedCardRecord, CardIngestJobRecord, OnlineEvidenceCardSearchRequestRecord,
    OnlineEvidenceCardUpdateRequestInput, OnlineEvidenceCardUpdateRequestRecord,
    OnlineEvidenceCardWorkerRunInput, OnlineEvidenceCardWorkerRunReport,
    RawEvidenceCandidateRecord, add_online_evidence_retrieval_repair_search_requests,
...
pub use online_learning::{
    ONLINE_LEARNING_PROMPT_CANDIDATE_SCHEMA_VERSION, ONLINE_LEARNING_TRACE_SCHEMA_VERSION,
    OnlineLearningTraceSummary, TieredEvidenceBinding,
    list_online_learning_prompt_candidates_for_trace,
};
pub use promotion_manifest::{
    ONLINE_LEARNING_PROMOTION_MANIFEST_SCHEMA_VERSION, PromotionManifestInput,
    list_online_learning_promotion_manifests_for_trace, read_online_learning_promotion_manifest,
    record_online_learning_promotion_manifest,
};
pub use runtime_rule_candidates::{
    RUNTIME_RULE_CANDIDATE_TYPES, RuntimeRuleCandidateActiveMatch,
    RuntimeRuleCandidatePromotionInput, RuntimeRuleCandidatePromotionPatch,
    RuntimeRuleCandidatePromotionPaths, active_runtime_rule_candidate_matches,
    is_runtime_rule_candidate_type, promote_runtime_rule_candidate_to_catalog,
...
pub struct EvidencePackage {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub online_learning_trace: Option<OnlineLearningTraceSummary>,
...
pub const ANSWER_RULES_PATH_ENV: &str = "TONGLINGYU_ANSWER_RULES_PATH";
pub const CORPUS_TOPOLOGY_PATH_ENV: &str = "TONGLINGYU_CORPUS_TOPOLOGY_PATH";
pub const RUNTIME_PROMPT_CATALOG_PATH_ENV: &str = "TONGLINGYU_RUNTIME_PROMPT_CATALOG_PATH";
...
pub fn runtime_rule_catalog_metadata() -> Result<Value> {
    Ok(json!({
        ...
        "answer_rules": answer_rules::answer_rule_catalog_metadata()?,
        "corpus_topology": corpus_topology::corpus_topology_catalog_metadata()?,
        "runtime_prompt_catalog": prompt_catalog::runtime_prompt_catalog_metadata()?,
    }))
}
...
static QUERY_EXPANSION_CATALOG_CACHE: OnceLock<
    Mutex<rule_catalog::RuleFileCache<QueryExpansionCatalog>>,
> = OnceLock::new();
...
impl TonglingyuRuntimeStore {
    ...
    async fn execute_workflow_with_agent_runtime_client_inner(
        ...
        let online_learning_assets = online_learning::record_agent_runtime_online_learning_assets(
            &conn,
            &workflow,
            mode.as_str(),
            retrieval_repair_search_request_count,
        )?;
        if retrieval_repair_search_request_count > 0 {
            append_runtime_audit_event(
                &conn,
                &workflow.trace_id,
                "agent_runtime_retrieval_repair_search_requests_recorded",
                &json!({
                    "runtime_mode": mode.as_str(),
                    "search_request_count": retrieval_repair_search_request_count,
                    "local_evidence_card_ingest_enqueued": true,
                    "online_learning_assets_recorded": online_learning_assets.is_some(),
                }),
            )?;
        }
    ...
    fn record_agent_runtime_rejection(
        ...
        if let Ok(conn) = self.open_connection() {
            ...
            let _ = online_learning::record_agent_runtime_prompt_failure_candidate(
                &conn,
                workflow,
                failure_stage,
                error,
    ...
    pub fn rule_catalog_metadata(&self) -> Result<Value> {
        runtime_rule_catalog_metadata()
    }
    ...
    pub fn online_evidence_card_search_requests_for_trace(
        &self,
        trace_id: &str,
        limit: usize,
    ) -> Result<Vec<OnlineEvidenceCardSearchRequestRecord>> {
        let conn = self.open_connection()?;
        online_evidence_card_ingest::list_online_evidence_card_search_requests_for_trace(
            &conn, trace_id, limit,
        )
    }
    ...
    pub fn online_learning_prompt_candidates_for_trace(
        &self,
        trace_id: &str,
        limit: usize,
    ) -> Result<Vec<Value>> {
        let conn = self.open_connection()?;
        online_learning::list_online_learning_prompt_candidates_for_trace(&conn, trace_id, limit)
    }

    pub fn online_learning_promotion_manifests_for_trace(
        &self,
        trace_id: &str,
        limit: usize,
    ) -> Result<Vec<Value>> {
        let conn = self.open_connection()?;
        promotion_manifest::list_online_learning_promotion_manifests_for_trace(
            &conn, trace_id, limit,
        )
    }
...
pub fn execute_runtime_workflow(
    ...
    let required_evidence_types = runtime_required_evidence_types(
        &input.question,
        &input.required_evidence_types,
    ...
    let package = create_evidence_package_with_question_frame(
        ...
        Some(serde_json::to_value(&source_scope_report.policy)?),
    )?;
    ...
    let online_learning_candidate_refs = online_learning::record_online_learning_candidate_refs(
        conn,
        &input.trace_id,
        package.online_learning_trace.as_ref(),
        online_evidence_card_update_request.as_ref(),
    )?;
    ...
    steps.push(workflow_step_report(
        ...
        WorkflowStepReportInput {
            ...
            "tiered_evidence_binding_count": package.tiered_evidence_bindings.len(),
            "evidence_tiers": package.tiered_evidence_bindings.iter().map(|binding| binding.evidence_tier.clone()).collect::<BTreeSet<_>>(),
            "online_learning_trace_id": package.online_learning_trace.as_ref().map(|trace| trace.online_learning_trace_id.clone()),
            ...
            "online_learning_candidate_refs": online_learning_candidate_refs,
            }),
            context: &input.context,
        },
    )?);
    ...
    steps.push(workflow_step_report(
        ...
        WorkflowStepReportInput {
            ...
            "evidence_brief": upstream_evidence_brief_for_package(
                &input.question,
                &package,
                question_frame.as_ref(),
            ),
            "answer_requirements": answer_requirements_for_message(&input.question, &package.cards)?,
...
async fn attach_agent_runtime_step_execution(
    ...
    let mut local_governance_skips = Vec::new();
    for (index, step) in workflow.steps.iter().cloned().enumerate() {
        if !agent_runtime_profile_step_enabled(mode, &step.operation) {
            continue;
        }
        if let Some(skipped_execution) =
            agent_runtime_profile_step_local_governance_skip(index, mode, workflow, &step)?
        {
            local_governance_skips.push(skipped_execution);
            continue;
        }
    ...
    for execution in local_governance_skips {
...
fn agent_runtime_profile_step_local_governance_skip(
    index: usize,
    mode: TonglingyuAgentRuntimeMode,
    workflow: &RuntimeWorkflowOutput,
...
async fn execute_agent_runtime_profile_step(
    ...
    let execution = execute_agent_runtime_profile_step_with_retry(
        ...
        visible_question,
        message_bytes,
        agent_runtime_profile_step_max_attempts(mode),
        runtime,
    )
    .await?;
...
async fn execute_agent_runtime_profile_step_with_retry(
    ...
    loop {
        ...
        match runtime.execute_profile_step(input).await {
            ...
            Err(error) if attempt_count < max_attempts => {
                retry_error_count += 1;
                attempt_count += 1;
                let _ = error;
            }
...
fn agent_runtime_profile_step_message(
    ...
    projection: &RuntimeContextProjection,
    result_summary_contract: &str,
) -> Result<RuntimeProfileMessage> {
    let max_compaction_level = agent_runtime_profile_message_max_compaction_level(step);
    ...
    Ok(RuntimeProfileMessage::new("user", selected_content))
}
...
fn agent_runtime_profile_step_message_content(
    ...
    let result_summary_contract = agent_runtime_result_summary_contract_for_message(
        result_summary_contract,
        step,
        compaction_level,
    )?;
...
fn agent_runtime_result_summary_contract_for_message(
    result_summary_contract: &str,
    step: &RuntimeWorkflowStepReport,
    compaction_level: usize,
) -> Result<String> {
    if step.operation == "draft_answer" && compaction_level >= 4 {
        return prompt_catalog::compact_draft_result_summary_contract();
    }
...
fn compact_answer_requirements_for_message(
    answer_requirements: Value,
    compaction_level: usize,
) -> Value {
    if compaction_level == 0 || !answer_requirements.is_object() {
        return answer_requirements;
    }
    let anchors = answer_requirements
        .get("requested_evidence_type_anchors")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    let compact_anchors = anchors
        .iter()
        .filter_map(Value::as_object)
        .map(|anchor| {
            let mut compact_anchor = Map::new();
            insert_existing_value(&mut compact_anchor, anchor, "evidence_type");
            insert_existing_value(&mut compact_anchor, anchor, "public_label");
            insert_existing_value(&mut compact_anchor, anchor, "required_cue_kind");
            let card_limit = if compaction_level >= 4 {
                1
            } else if compaction_level >= 2 {
                2
            } else {
                3
            };
            let cue_limit = if compaction_level >= 4 {
                2
            } else if compaction_level >= 2 {
                3
            } else {
            ...
            let title_chars = if compaction_level >= 4 {
                28
            } else if compaction_level >= 2 {
                48
            } else {
            ...
            let cue_chars = if compaction_level >= 4 {
                18
            } else if compaction_level >= 2 {
                24
            } else {
                ...
                .map(|items| {
                    Value::Array(
                        items
                            ...
                            .take(card_limit)
                            .map(|card| {
                                let mut compact_card = Map::new();
                                insert_existing_value(&mut compact_card, card, "evidence_id");
                                insert_existing_value(&mut compact_card, card, "source_layer");
                                ...
                                compact_card.insert(
                                    "acceptable_anchor_cues".to_string(),
                                    compact_string_array_for_message(
                                        card.get("acceptable_anchor_cues").unwrap_or(&Value::Null),
                                        cue_limit,
                                        cue_chars,
                                    ),
                                );
                                compact_card.insert(
                                    "source_anchor_cues".to_string(),
                                    compact_source_cues_for_message(
                                        card.get("source_anchor_cues").unwrap_or(&Value::Null),
                                        cue_limit,
                                        cue_chars,
                                    ),
                                );
                                compact_card.insert(
                                    "text_anchor_cues".to_string(),
                                    compact_string_array_for_message(
                                        card.get("text_anchor_cues").unwrap_or(&Value::Null),
                                        cue_limit,
                                        cue_chars,
                                    ),
                                );
                                Value::Object(compact_card)
                            })
                            .collect(),
                    )
                })
                .unwrap_or_else(|| json!([]));
            compact_anchor.insert("cards".to_string(), cards);
    ...
    json!({
        "object": answer_requirements
            .get("object")
            .cloned()
            .unwrap_or_else(|| json!("tonglingyu.answer_requirements")),
        "schema_version": answer_requirements
            .get("schema_version")
            .cloned()
            .unwrap_or(Value::Null),
        "evidence_request": answer_requirements
...
fn compact_question_frame_answer_requirements_for_message(
    requirements: Value,
    compaction_level: usize,
) -> Value {
    let chapter_location = requirements
        ...
        .map(|chapter| {
            ...
            for key in [
                "event",
                "must_answer_chapter_no",
                "chapter_title",
                "primary_evidence_id",
                "answer_shape",
                "commentary_visibility_rule",
            ] {
                insert_existing_value(&mut compact, chapter, key);
            }
            insert_trimmed_string(
                &mut compact,
                chapter,
                "primary_source_title",
                if compaction_level >= 3 { 32 } else { 56 },
            );
            insert_trimmed_string(
                &mut compact,
                chapter,
                "primary_text_cue",
                if compaction_level >= 3 { 56 } else { 96 },
            );
            compact.insert(
                "primary_source_cues".to_string(),
                compact_source_cues_for_message(
                    chapter.get("primary_source_cues").unwrap_or(&Value::Null),
                    if compaction_level >= 3 { 2 } else { 3 },
                    if compaction_level >= 3 { 18 } else { 24 },
                ),
            );
            if compaction_level <= 2 {
                insert_trimmed_string(&mut compact, chapter, "rule", 180);
            } else {
                compact.insert(
                    "rule_id".to_string(),
                    json!("chapter_location.direct_first"),
                );
            }
            Value::Object(compact)
        });
    let relation_open_object = requirements
        .get("relation_open_object")
        .and_then(|value| value.get("must_mention_objects"))
        .and_then(Value::as_array);
    if compaction_level == 0 {
        return requirements;
    }
    let mut root = Map::new();
    if let Some(chapter_location) = chapter_location {
        root.insert("chapter_location".to_string(), chapter_location);
    }
    let Some(objects) = relation_open_object else {
    ...
    let (cue_limit, cue_chars, source_cue_limit, source_cue_chars, title_chars, text_chars) =
        match compaction_level {
            1 => (6, 18, 3, 24, 48, 96),
            2 => (4, 16, 2, 20, 40, 72),
            3 => (3, 14, 2, 18, 32, 48),
    ...
    root.insert(
        "relation_open_object".to_string(),
        json!({
            "must_mention_objects": objects
                .iter()
                .filter_map(Value::as_object)
                .map(|object| {
                    let mut compact = Map::new();
                    insert_existing_value(&mut compact, object, "canonical");
                    insert_existing_value(&mut compact, object, "evidence_id");
                    compact.insert(
                        "acceptable_cues".to_string(),
                        compact_string_array_for_message(
                            object.get("acceptable_cues").unwrap_or(&Value::Null),
                            cue_limit,
                            cue_chars,
                        ),
                    );
                    compact.insert(
                        ...
                        compact_source_cues_for_message(
                            object.get("source_cues").unwrap_or(&Value::Null),
                            source_cue_limit,
                            source_cue_chars,
                        ),
                    );
                    insert_trimmed_string(&mut compact, object, "source_title", title_chars);
                    insert_trimmed_string(&mut compact, object, "text_cue", text_chars);
                    Value::Object(compact)
                })
                .collect::<Vec<_>>(),
        }),
...
fn compact_entity_intro_answer_policy_for_message(policy: Value, compaction_level: usize) -> Value {
    if compaction_level == 0 || !policy.is_object() {
        return policy;
    }
    ...
    let mut compact = Map::new();
    insert_existing_value(&mut compact, policy_obj, "object");
    insert_existing_value(&mut compact, policy_obj, "applies");
    if let Some(entity) = policy.get("entity").and_then(Value::as_object) {
        let mut compact_entity = Map::new();
        insert_existing_value(&mut compact_entity, entity, "canonical");
        compact_entity.insert(
            "aliases".to_string(),
            compact_string_array_for_message(
                entity.get("aliases").unwrap_or(&Value::Null),
                if compaction_level >= 4 { 2 } else { 4 },
                if compaction_level >= 4 { 10 } else { 16 },
            ),
        );
        compact.insert("entity".to_string(), Value::Object(compact_entity));
    }
    insert_existing_value(&mut compact, policy_obj, "min_supporting_cards");
    insert_existing_value(&mut compact, policy_obj, "max_supporting_cards");
    insert_existing_value(&mut compact, policy_obj, "short_speech_shell_max_chars");
    if compaction_level <= 3 {
        compact.insert(
            "excluded_public_quote_terms".to_string(),
            compact_string_array_for_message(
                policy
                    .get("excluded_public_quote_terms")
                    .unwrap_or(&Value::Null),
                3,
                18,
            ),
        ...
        compact.insert(
            "excluded_public_quote_term_count".to_string(),
            json!(
                policy
                    .get("excluded_public_quote_terms")
                    .and_then(Value::as_array)
    ...
    if compaction_level <= 2 {
        if let Some(rule) = policy.get("rule").and_then(Value::as_str) {
            compact.insert("rule".to_string(), Value::String(trim_text(rule, 160)));
        }
    } else {
        compact.insert(
            "rule_id".to_string(),
            json!("entity_intro.multiple_readable_cards_no_external_biography"),
        );
    }
    Value::Object(compact)
}
...
fn compact_source_cues_for_message(value: &Value, max_items: usize, max_chars: usize) -> Value {
    let Some(items) = value.as_array() else {
        return json!([]);
    };
    let mut cues = items
    ...
    cues.sort_by_key(|cue| {
        (
            source_cue_compaction_rank(cue),
            cue.chars().count(),
            cue.to_string(),
        )
    });
...
fn source_cue_compaction_rank(cue: &str) -> usize {
    let normalized = normalize_text(cue);
    let char_count = normalized.chars().count();
    if char_count <= 12 && normalized.contains('第') && normalized.contains('回') {
        0
    } else if char_count <= 24 && normalized.contains('第') && normalized.contains('回') {
        1
...
fn step_output_message_payload_with_compaction(
    ...
    let answer_requirements = step
        .output
        .get("answer_requirements")
        .cloned()
        .unwrap_or_else(|| json!({}));
    let answer_requirements = if step.operation == "draft_answer" {
        compact_answer_requirements_for_message(answer_requirements, compaction_level)
    } else {
        answer_requirements
    };
    let question_frame_answer_requirements = step
        .output
        .get("question_frame_answer_requirements")
        .cloned()
        .unwrap_or_else(|| json!({}));
    let question_frame_answer_requirements = if step.operation == "draft_answer" {
        compact_question_frame_answer_requirements_for_message(
            question_frame_answer_requirements,
            compaction_level,
        )
    } else {
        question_frame_answer_requirements
    };
    let entity_intro_answer_policy = step
        .output
        .get("entity_intro_answer_policy")
        .cloned()
        .unwrap_or_else(|| json!({}));
    let entity_intro_answer_policy = if step.operation == "draft_answer" {
        compact_entity_intro_answer_policy_for_message(entity_intro_answer_policy, compaction_level)
    } else {
        entity_intro_answer_policy
    };
    let evidence_ids = if step.operation == "draft_answer" {
        evidence_ids_from_evidence_brief_and_answer_requirements(
            &evidence_brief,
            &answer_requirements,
            &question_frame_answer_requirements,
        )
        .unwrap_or(output_evidence_ids)
    } else {
        output_evidence_ids
    };
...
fn evidence_ids_from_evidence_brief_and_answer_requirements(
    evidence_brief: &Value,
    answer_requirements: &Value,
    question_frame_answer_requirements: &Value,
) -> Option<Value> {
    let mut ids = evidence_ids_from_evidence_brief(evidence_brief)?
        .as_array()
        .cloned()
        .unwrap_or_default()
        .into_iter()
        ...
        .collect::<Vec<_>>();
    if let Some(requirements) = answer_requirements
        .get("requested_evidence_type_anchors")
        .and_then(Value::as_array)
    {
        for requirement in requirements {
            let Some(cards) = requirement.get("cards").and_then(Value::as_array) else {
                continue;
            };
            for card in cards {
                let Some(evidence_id) = card.get("evidence_id").and_then(Value::as_str) else {
                    continue;
                };
                push_unique_text(&mut ids, evidence_id);
            }
        }
    }
    if let Some(requirements) = question_frame_answer_requirements
        .get("relation_open_object")
        .and_then(|value| value.get("must_mention_objects"))
        .and_then(Value::as_array)
    {
        for requirement in requirements {
            let Some(evidence_id) = requirement.get("evidence_id").and_then(Value::as_str) else {
                continue;
            };
            push_unique_text(&mut ids, evidence_id);
        }
    ...
    for evidence_id in
        chapter_location_evidence_ids_for_requirements(question_frame_answer_requirements)
    {
        push_unique_text(&mut ids, &evidence_id);
    }
    Some(Value::Array(
...
fn agent_runtime_result_summary_contract(step: &RuntimeWorkflowStepReport) -> Result<String> {
    prompt_catalog::result_summary_contract_for_operation(&step.operation)
}
...
fn reject_agent_runtime_draft_application(
    ...
    if let Some(step) = workflow.steps.get_mut(draft_step_index) {
        ...
        step.output["agent_runtime_package_id_rebound"] = json!(extraction.package_id_rebound);
        step.output["agent_runtime_observed_bundle_package_id"] =
            json!(&extraction.observed_bundle_package_id);
        step.output["agent_runtime_observed_candidate_package_id"] =
            json!(&extraction.observed_candidate_package_id);
        ...
        step.output["agent_runtime_retrieval_repair_query_count"] =
            json!(extraction.retrieval_repair_queries.len());
        step.output["agent_runtime_retrieval_repair_queries"] =
            json!(&extraction.retrieval_repair_queries);
        ...
        if let Some(agent_runtime) = step.agent_runtime.as_mut().and_then(Value::as_object_mut) {
            ...
            agent_runtime.insert(
                ...
                json!({
                    ...
                    "package_id_rebound": extraction.package_id_rebound,
                    "observed_bundle_package_id": &extraction.observed_bundle_package_id,
                    "observed_candidate_package_id": &extraction.observed_candidate_package_id,
                    "retrieval_repair_recommended": extraction.retrieval_repair_recommended,
                    "retrieval_repair_query_count": extraction.retrieval_repair_queries.len(),
                    "draft_consumed": false,
...
fn agent_runtime_retrieval_repair_queries(workflow: &RuntimeWorkflowOutput) -> Vec<Value> {
    workflow
        .steps
        .iter()
...
fn record_agent_runtime_retrieval_repair_search_requests(
    conn: &Connection,
    workflow: &RuntimeWorkflowOutput,
) -> Result<usize> {
    let retrieval_repair_queries = agent_runtime_retrieval_repair_queries(workflow);
    if retrieval_repair_queries.is_empty() {
        return Ok(0);
    }
    let source_scope_policy =
        serde_json::to_value(source_scope_policy_for_question(&workflow.question))?;
    online_evidence_card_ingest::add_online_evidence_retrieval_repair_search_requests(
        conn,
        &workflow.trace_id,
        &workflow.question,
...
fn should_repair_agent_runtime_draft(
    ...
    agent_runtime_profile_observation_mode(mode)
        ...
        && !application.is_some_and(|value| {
            value
                .rejected_reason
                .is_some_and(|reason| reason.starts_with("question_frame_relation_"))
        })
        && !application.is_some_and(|value| {
            value
                .rejected_reason
                .is_some_and(agent_runtime_draft_rejection_skips_repair)
        })
}
...
fn agent_runtime_draft_governance_decision_is_complete(
    ...
    application.draft_consumed
        || application
            .rejected_reason
            .is_some_and(agent_runtime_draft_rejection_is_governed_decision)
}

fn agent_runtime_draft_rejection_is_governed_decision(reason: &str) -> bool {
    matches!(
        reason,
        "coverage_assessment_not_passed"
            | "coverage_assessment_status_missing"
            | "claim_statements_missing"
            | "claim_statements_empty"
            | "claim_statement_invalid"
            | "claim_statement_text_missing"
            | "claim_evidence_refs_missing"
            ...
            | "question_frame_relation_boundary_missing"
    )
}
...
async fn repair_agent_runtime_draft(
    ...
    let projection = context
        ...
        .clone();
    let result_summary_contract = agent_runtime_result_summary_contract(&draft_step)?;
...
fn agent_runtime_profile_step_repair_message_content(
    ...
    let step_output = step_output_message_payload_with_compaction(step, compaction_level);
    let repair_context = prompt_catalog::draft_repair_context(rejected_reason, compaction_level)?;
    let result_summary_contract = draft_repair_result_summary_contract_for_message(
        result_summary_contract,
        compaction_level,
    )?;
    let repair_instruction = prompt_catalog::draft_repair_instruction()?;
    Ok(format!(
...
fn draft_repair_result_summary_contract_for_message(
    result_summary_contract: &str,
    compaction_level: usize,
) -> Result<String> {
    if compaction_level < 4 {
        return Ok(result_summary_contract.to_string());
    }
    prompt_catalog::compact_draft_result_summary_contract()
}
...
fn agent_runtime_llm_call_budget_audit(
    mode: TonglingyuAgentRuntimeMode,
    workflow: &RuntimeWorkflowOutput,
    executed_profile_step_count: usize,
) -> Value {
    let step_latencies = workflow
        .steps
        .iter()
        .map(agent_runtime_step_latency_audit)
        .collect::<Vec<_>>();
    let provider_latency_ms = step_latencies
        .iter()
        .filter_map(|step| step.get("provider_latency_ms").and_then(Value::as_u64))
        .sum::<u64>();
    let max_step_duration_ms = step_latencies
        .iter()
        .filter_map(|step| step.get("duration_ms").and_then(Value::as_u64))
        .max()
        .unwrap_or(0);
    ...
    let skipped_profile_operations = workflow
        ...
        .filter(|step| {
            step.agent_runtime
                .as_ref()
                .is_none_or(|value| value.get("status") != Some(&json!("executed")))
        })
        .map(|step| step.operation.clone())
        .collect::<Vec<_>>();
    let call_budget_exceeded = sync_llm_call_count > AGENT_RUNTIME_SYNC_LLM_CALL_BUDGET;
    let latency_budget_exceeded = provider_latency_ms > AGENT_RUNTIME_SYNC_LATENCY_BUDGET_MS;
    json!({
        "schema_version": "tonglingyu-agent-runtime-llm-call-budget-v1",
        "scope": "agent_runtime_profiles",
        "runtime_mode": mode.as_str(),
        "sync_execution_policy": agent_runtime_sync_execution_policy(mode),
        "default_sync_call_budget": AGENT_RUNTIME_SYNC_LLM_CALL_BUDGET,
        "default_sync_latency_budget_ms": AGENT_RUNTIME_SYNC_LATENCY_BUDGET_MS,
        "sync_llm_call_count": sync_llm_call_count,
        "provider_latency_ms": provider_latency_ms,
        "max_step_duration_ms": max_step_duration_ms,
        "step_latencies": step_latencies,
        "skipped_profile_step_count": skipped_profile_operations.len(),
        "skipped_profile_operations": skipped_profile_operations,
        "call_budget_exceeded": call_budget_exceeded,
        "latency_budget_exceeded": latency_budget_exceeded,
        "latency_regression": call_budget_exceeded || latency_budget_exceeded,
        "normal_sync_budget_enforced": !call_budget_exceeded,
    })
}

fn agent_runtime_step_latency_audit(step: &RuntimeWorkflowStepReport) -> Value {
    json!({
        "step_id": &step.step_id,
        "profile": &step.profile,
        "operation": &step.operation,
        "duration_ms": step.duration_ms,
        "provider_latency_ms": step
            .agent_runtime
            .as_ref()
        ...
        "attempt_count": step
            ...
            .cloned()
            .unwrap_or(Value::Null),
        "retry_error_count": step
            .agent_runtime
            .as_ref()
            .and_then(|value| value.get("retry_error_count"))
            .cloned()
            .unwrap_or(Value::Null),
    })
}
...
fn agent_runtime_execution_summary(
    ...
    let executed_profile_step_count = workflow
        ...
        .count();
    let llm_call_budget =
        agent_runtime_llm_call_budget_audit(mode, workflow, executed_profile_step_count);
    ...
    let draft_skipped_by_local_governance = application.is_some_and(|value| {
        value
            .rejected_reason
            .is_some_and(agent_runtime_draft_rejection_skips_repair)
    });
    ...
    json!({
        ...
        "package_governance_completed": package_governance_completed,
        "draft_consumed": draft_consumed,
        "draft_skipped_by_local_governance": draft_skipped_by_local_governance,
        ...
        "reviewer_governance_completed": reviewer_governance_completed,
...
fn apply_agent_runtime_content_outputs(
    ...
    if let Some(reason) =
        agent_runtime_draft_evidence_tier_rejection(&extraction, &workflow.package)
    {
        return Some(reject_agent_runtime_draft_application(
            workflow,
            draft_step_index,
            ...
            &extraction,
            reason,
            "profile-evidence-tier-rejected",
        ));
    }
    ...
    if let Some(reason) = relation_draft_rejection_reason(
    ...
    if let Some(reason) = relation_open_object_draft_rejection_reason(
        parsed_question_frame.as_ref(),
        &workflow.package.cards,
        &draft,
    ) {
    ...
    if let Some(reason) = chapter_location_draft_rejection_reason(
        parsed_question_frame.as_ref(),
        &workflow.package.cards,
        &draft,
    ) {
...
fn agent_runtime_draft_evidence_tier_rejection(
    extraction: &UpstreamBundleDraftExtraction,
...
fn agent_runtime_draft_claim_evidence_support_rejection(
    ...
    if let Some(draft_answer) = extraction.draft_answer.as_deref() {
        ...
        if !draft_text_quoted_phrases_supported(draft_answer, &all_cards, 6) {
...
fn draft_claim_refs_textually_support(claim: &str, cards: &[&EvidenceCard]) -> bool {
    ...
    if !draft_text_quoted_phrases_supported_by_text(claim, &evidence_text, 3) {
        return false;
    }
    let claim_text = normalize_text(claim);
...
fn draft_text_quoted_phrases_supported(
    text: &str,
    cards: &[&EvidenceCard],
    min_cjk_count: usize,
) -> bool {
    let evidence_text = normalize_text(&draft_claim_support_text(cards));
    draft_text_quoted_phrases_supported_by_text(text, &evidence_text, min_cjk_count)
}

fn draft_text_quoted_phrases_supported_by_text(
    text: &str,
    evidence_text: &str,
    min_cjk_count: usize,
) -> bool {
    draft_text_quoted_phrases(text, min_cjk_count)
        .iter()
        .all(|phrase| evidence_text.contains(phrase))
}

fn draft_text_quoted_phrases(text: &str, min_cjk_count: usize) -> Vec<String> {
    let pairs = [
        ('“', '”'),
        ('「', '」'),
        ('『', '』'),
...
fn draft_quoted_phrase_is_substantive(phrase: &str, min_cjk_count: usize) -> bool {
    ...
    let alnum_count = phrase
        ...
        .count();
    cjk_count >= min_cjk_count || alnum_count >= 4
}
            ...
            metadata: json!({
                ...
                "agent_runtime": step.agent_runtime.as_ref().map(|value| json!({
                    ...
                    "content_used_for_final_answer": value
                        ...
                        .cloned()
                        .unwrap_or(Value::Null),
                    "latency_ms": value.get("latency_ms").cloned().unwrap_or(Value::Null),
...
fn apply_runtime_schema(conn: &Connection) -> Result<()> {
    ...
    online_learning::init_schema(conn)?;
    promotion_manifest::init_schema(conn)?;
...
pub fn runtime_store_stats(conn: &Connection) -> Result<RuntimeStoreStats> {
    Ok(RuntimeStoreStats {
        ...
        evidence_packages: table_count_if_exists(conn, "evidence_packages")?,
        evidence_cards: table_count_if_exists(conn, "evidence_cards")?,
        retrieval_failures: table_count_if_exists(conn, "retrieval_failures")?,
        governance_tasks: table_count_if_exists(conn, "knowledge_governance_tasks")?,
        knowledge_patch_proposals: table_count_if_exists(conn, "knowledge_patch_proposals")?,
        knowledge_items: table_count_if_exists(conn, "knowledge_items")?,
        knowledge_item_state_history: table_count_if_exists(conn, "knowledge_item_state_history")?,
        knowledge_calibration_reports: table_count_if_exists(
            conn,
            "knowledge_calibration_reports",
        )?,
        knowledge_calibration_jobs: table_count_if_exists(conn, "knowledge_calibration_jobs")?,
        knowledge_calibration_job_history: table_count_if_exists(
            conn,
            "knowledge_calibration_job_history",
        )?,
        audit_events: table_count_if_exists(conn, "audit_events")?,
        review_status: grouped_count_map_if_table_exists(
            conn,
            "evidence_packages",
            "SELECT review_status, COUNT(*) FROM evidence_packages GROUP BY review_status",
        )?,
        evidence_types: grouped_count_map_if_table_exists(
            conn,
            "evidence_cards",
            "SELECT evidence_type, COUNT(*) FROM evidence_cards GROUP BY evidence_type",
        )?,
        retrieval_failure_status: grouped_count_map_if_table_exists(
            conn,
            "retrieval_failures",
            "SELECT human_review_status, COUNT(*) FROM retrieval_failures GROUP BY human_review_status",
        )?,
        retrieval_failure_type: grouped_count_map_if_table_exists(
            conn,
            "retrieval_failures",
            "SELECT failure_type, COUNT(*) FROM retrieval_failures GROUP BY failure_type",
        )?,
        governance_task_status: grouped_count_map_if_table_exists(
            conn,
            "knowledge_governance_tasks",
            "SELECT status, COUNT(*) FROM knowledge_governance_tasks GROUP BY status",
        )?,
        governance_task_type: grouped_count_map_if_table_exists(
            conn,
            "knowledge_governance_tasks",
            "SELECT task_type, COUNT(*) FROM knowledge_governance_tasks GROUP BY task_type",
        )?,
        governance_task_priority: grouped_count_map_if_table_exists(
            conn,
            "knowledge_governance_tasks",
            "SELECT priority, COUNT(*) FROM knowledge_governance_tasks GROUP BY priority",
        )?,
        knowledge_item_state: grouped_count_map_if_table_exists(
            conn,
            "knowledge_items",
            "SELECT state, COUNT(*) FROM knowledge_items GROUP BY state",
        )?,
        knowledge_item_kind: grouped_count_map_if_table_exists(
            conn,
            "knowledge_items",
            "SELECT kind, COUNT(*) FROM knowledge_items GROUP BY kind",
        )?,
        knowledge_calibration_report_method: grouped_count_map_if_table_exists(
            conn,
            "knowledge_calibration_reports",
            "SELECT method, COUNT(*) FROM knowledge_calibration_reports GROUP BY method",
        )?,
        knowledge_calibration_report_decision: grouped_count_map_if_table_exists(
            conn,
            "knowledge_calibration_reports",
            "SELECT decision, COUNT(*) FROM knowledge_calibration_reports GROUP BY decision",
        )?,
        knowledge_calibration_job_status: grouped_count_map_if_table_exists(
            conn,
            "knowledge_calibration_jobs",
            "SELECT status, COUNT(*) FROM knowledge_calibration_jobs GROUP BY status",
        )?,
        audit_event_types: grouped_count_map_if_table_exists(
            conn,
            "audit_events",
...
fn grouped_count_map_if_table_exists(
    conn: &Connection,
    table: &str,
    sql: &str,
) -> Result<BTreeMap<String, i64>> {
    if sqlite_table_exists(conn, table)? {
        grouped_count_map(conn, sql)
    } else {
        Ok(BTreeMap::new())
    }
}
...
pub fn create_evidence_package(
    ...
    create_evidence_package_with_question_frame(conn, trace_id, question, cards, None, None)
}
...
fn create_evidence_package_with_question_frame(
    ...
    cards: Vec<EvidenceCard>,
    question_frame: Option<Value>,
    source_scope_policy: Option<Value>,
) -> Result<EvidencePackage> {
    let package_id = format!("pkg-{}", uuid::Uuid::now_v7().simple());
    let cards = package_scoped_evidence_cards(conn, &package_id, cards)?;
    ...
    let source_scope_policy = match source_scope_policy {
        Some(value) => value,
        None => serde_json::to_value(source_scope_policy_for_question(question))?,
    };
    let mut online_learning_trace = online_learning::build_online_learning_trace_summary(
        conn,
        &package_id,
        trace_id,
        &cards,
    ...
    apply_evidence_tier_review(
        &mut review,
        &claim_evidence_map,
        &online_learning_trace.tiered_evidence_bindings,
    );
    online_learning_trace = online_learning::build_online_learning_trace_summary(
        conn,
        &package_id,
        trace_id,
        &cards,
    ...
    let tiered_evidence_bindings = online_learning_trace.tiered_evidence_bindings.clone();
    let now = now_rfc3339();
    ...
    append_runtime_audit_event(
        ...
        &json!({
            ...
            "tiered_evidence_bindings": &tiered_evidence_bindings,
    ...
    append_runtime_audit_event(
        conn,
        trace_id,
        "online_learning_trace_recorded",
        &serde_json::to_value(&online_learning_trace)?,
    )?;
    append_runtime_audit_event(
        conn,
        trace_id,
    ...
    Ok(EvidencePackage {
        ...
        question_frame,
        tiered_evidence_bindings,
        online_learning_trace: Some(online_learning_trace),
        review,
    })
}

fn package_scoped_evidence_cards(
    conn: &Connection,
    package_id: &str,
    cards: Vec<EvidenceCard>,
) -> Result<Vec<EvidenceCard>> {
    let mut scoped = Vec::with_capacity(cards.len());
    let mut seen_ids = HashSet::new();
    for (index, mut card) in cards.into_iter().enumerate() {
        let original_id = card.evidence_id.clone();
        let id_already_used =
            !seen_ids.insert(original_id.clone()) || evidence_card_id_exists(conn, &original_id)?;
        if id_already_used {
            card.evidence_id = package_scoped_evidence_id(package_id, index, &card);
            seen_ids.insert(card.evidence_id.clone());
        }
        scoped.push(card);
    }
    Ok(scoped)
...
fn evidence_card_id_exists(conn: &Connection, evidence_id: &str) -> Result<bool> {
    Ok(conn
        .query_row(
            "SELECT 1 FROM evidence_cards WHERE evidence_id = ?1 LIMIT 1",
            params![evidence_id],
            |_| Ok(()),
        )
        .optional()?
...
fn package_scoped_evidence_id(package_id: &str, index: usize, card: &EvidenceCard) -> String {
    format!(
        "evpkg-{}",
        &hash_text(&format!(
            "{}:{}:{}:{}:{}",
            package_id, index, card.evidence_id, card.source_id, card.block_id
        ))[..32]
    )
}
...
pub fn load_evidence_package_from_conn(
    ...
    let package: Option<EvidencePackageRow> = conn
        .query_row(
    ...
    let review: ReviewRecord = serde_json::from_str(&review_json)?;
    let source_scope_policy = serde_json::to_value(source_scope_policy_for_question(&question))?;
    let online_learning_trace = online_learning::build_online_learning_trace_summary(
        conn,
        &package_id,
        &trace_id,
        &cards,
...
fn runtime_required_evidence_types(
    question: &str,
    fallback: &[String],
    frame: Option<&question_frame::RuntimeQuestionFrame>,
) -> Vec<String> {
    let mut required = relation_required_evidence_types(fallback, frame);
    if let Some(frame) = frame {
        for item in &frame.required_evidence_types {
            push_term(&mut required, item);
        }
    }
    let asks_character_fate = frame
        .map(|frame| frame.is_character_fate())
        .unwrap_or(false)
        || question_asks_for_fate(question);
    if asks_character_fate && !source_scope_policy_for_question(question).later_forty_allowed {
        push_term(&mut required, "commentary");
    }
    required
}
...
fn filter_cards_for_question_frame_answer_scope(
    question: &str,
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    cards: Vec<EvidenceCard>,
) -> Vec<EvidenceCard> {
...
fn question_frame_focus_search(
    ...
    blocks.sort_by(|left, right| {
        let left_rank = question_frame_focus_rank(question, left, &focus_terms);
        let right_rank = question_frame_focus_rank(question, right, &focus_terms);
        right_rank
            .cmp(&left_rank)
            .then_with(|| {
                relation_support_source_rank(left).cmp(&relation_support_source_rank(right))
            })
            .then_with(|| left.text.chars().count().cmp(&right.text.chars().count()))
            .then_with(|| left.block_id.cmp(&right.block_id))
    });
    blocks.truncate(limit);
    Ok(blocks.into_iter().map(evidence_card_from_block).collect())
}

fn question_frame_focus_rank(
    question: &str,
    block: &SearchBlockRecord,
    focus_terms: &[String],
) -> i64 {
    let normalized_text = normalize_text(&block.text);
    let normalized_title = normalize_text(&block.source_title);
    let mut score = 0_i64;
    for term in focus_terms {
        let term = term.trim();
        if term.is_empty() {
        ...
        let normalized_term = normalize_text(term);
        if block.text.contains(term) || normalized_text.contains(&normalized_term) {
            score += 20 + term.chars().count() as i64;
        }
        if block.source_title.contains(term) || normalized_title.contains(&normalized_term) {
    ...
    if let Ok(ranking) = retrieval_rules::ranking_rules() {
        if retrieval_rules::contains_any_term(question, &ranking.fate_question_terms) {
            let fate_match_count = matching_rule_term_count(&block.text, &ranking.fate_text_terms);
            if fate_match_count > 0 {
                score += 45 * fate_match_count as i64;
            } else {
                score -= 20;
            }
        }
    ...
    if retrieval_rules::classify_evidence_type("", &block.source_id, &block.text)
        .is_ok_and(|evidence_type| evidence_type == "version_note")
        && question_asks_for_fate(question)
    {
        score -= 30;
    }
...
fn attribute_direct_support_search(
    conn: &Connection,
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    limit: usize,
) -> Result<Vec<EvidenceCard>> {
    let Some(frame) = frame.filter(|frame| frame.is_attribute()) else {
        return Ok(Vec::new());
    };
    let mut candidate_blocks = BTreeMap::<String, SearchBlockRecord>::new();
    let per_term_limit = limit.saturating_mul(64).max(64);
    for term in attribute_support_candidate_terms(frame) {
    ...
    let mut supported = candidate_blocks
        .into_values()
        .filter_map(|block| {
            let card = evidence_card_from_block(block.clone());
            attribute_card_support(frame, &card).map(|support| (support, block, card))
        })
        .collect::<Vec<_>>();
    supported.sort_by(|left, right| {
        let left_rank = attribute_support_rank(&left.0, &left.1);
        let right_rank = attribute_support_rank(&right.0, &right.1);
        right_rank
            .cmp(&left_rank)
            .then_with(|| {
                relation_support_source_rank(&left.1).cmp(&relation_support_source_rank(&right.1))
            })
            .then_with(|| {
                left.1
                    .text
                    .chars()
                    .count()
                    .cmp(&right.1.text.chars().count())
            })
            .then_with(|| left.1.block_id.cmp(&right.1.block_id))
    });
    supported.truncate(limit);
    Ok(supported.into_iter().map(|(_, _, card)| card).collect())
}

fn attribute_support_candidate_terms(frame: &question_frame::RuntimeQuestionFrame) -> Vec<String> {
    let mut terms = Vec::new();
    if let Some(subject) = &frame.subject {
        for term in subject.identity_terms() {
    ...
    if let Some(predicate) = &frame.predicate {
        for term in predicate
            .aliases
            .iter()
            .chain(predicate.evidence_terms.iter())
            .chain(std::iter::once(&predicate.label))
        {
            if term.chars().count() >= 2 {
                push_term(&mut terms, term);
            }
...
fn attribute_support_rank(
    support: &question_frame::AttributeCardSupport,
    block: &SearchBlockRecord,
) -> i64 {
    let mut score = 80_i64;
    score += support.matched_terms.len() as i64 * 12;
    if support.evidence_strength == "direct" {
        score += 20;
    }
    if block.kind == "heading" {
        score -= 30;
...
fn relation_open_object_support_search(
    conn: &Connection,
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    limit: usize,
) -> Result<Vec<EvidenceCard>> {
    let search_terms = relation_open_object_search_terms(frame);
    ...
    let mut matched_blocks = candidate_blocks
        .into_values()
        .filter_map(|block| {
            let candidate_names = relation_open_object_text_candidate_names(frame, &block.text);
            if candidate_names.is_empty() {
                None
            } else {
                Some((candidate_names, block))
            }
        })
        .collect::<Vec<_>>();
    matched_blocks.sort_by_key(|(_, block)| {
        (
            relation_support_source_rank(block),
            block.text.chars().count(),
            block.block_id.clone(),
    ...
    for (candidate_names, block) in &matched_blocks {
        if candidate_names
            .iter()
            .any(|candidate| !covered_objects.contains(candidate))
            && selected_block_ids.insert(block.block_id.clone())
        {
            covered_objects.extend(candidate_names.iter().cloned());
            selected_blocks.push(block.clone());
            if selected_blocks.len() >= limit {
                break;
            }
        }
    }
    if selected_blocks.len() < limit {
        for (_, block) in matched_blocks {
            if selected_block_ids.insert(block.block_id.clone()) {
    ...
    let focus = frame
        .and_then(|frame| frame.subject.as_ref())
...
fn upstream_evidence_brief_for_package(
    question: &str,
    package: &EvidencePackage,
    frame: Option<&question_frame::RuntimeQuestionFrame>,
) -> Vec<Value> {
    let bindings_by_id = package
        ...
        .map(|binding| (binding.evidence_id.as_str(), binding))
        .collect::<BTreeMap<_, _>>();
    upstream_evidence_brief_for_cards(question, &package.cards, frame, &bindings_by_id)
}

#[cfg(test)]
fn upstream_evidence_brief_for_frame(
    question: &str,
    cards: &[EvidenceCard],
    frame: Option<&question_frame::RuntimeQuestionFrame>,
) -> Vec<Value> {
    upstream_evidence_brief_for_cards(question, cards, frame, &BTreeMap::new())
}

fn upstream_evidence_brief_for_cards(
    question: &str,
    cards: &[EvidenceCard],
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    bindings_by_id: &BTreeMap<&str, &TieredEvidenceBinding>,
) -> Vec<Value> {
    let mut focus_terms = upstream_evidence_focus_terms(question);
    for term in relation_open_object_focus_terms(frame, cards) {
        push_term(&mut focus_terms, &term);
    }
...
fn question_frame_answer_requirements_value(
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    cards: &[EvidenceCard],
) -> Result<Value> {
    ...
    payload.insert(
        "object".to_string(),
        json!("tonglingyu.question_frame_answer_requirements"),
    );
    payload.insert(
        "schema_version".to_string(),
        json!("tonglingyu.question_frame_answer_requirements.v1"),
    );
    if let Some(chapter_location) = chapter_location_answer_requirement_value(frame, cards) {
        payload.insert("chapter_location".to_string(), chapter_location);
    }
    let relation_objects = relation_open_object_supported_objects(frame, cards);
    if !relation_objects.is_empty() {
        let must_mention_objects = relation_objects
            .iter()
            .map(relation_open_object_requirement_value)
            .collect::<Result<Vec<_>>>()?;
        payload.insert(
            "relation_open_object".to_string(),
            json!({
                "must_mention_objects": must_mention_objects,
            }),
        );
    }
    if payload.len() == 2 {
...
fn relation_open_object_requirement_value(
    object: &question_frame::RelationOpenObjectSupport,
) -> Result<Value> {
    Ok(json!({
        "canonical": &object.canonical,
        "acceptable_cues": object
            .aliases
            .iter()
            .map(|alias| trim_text(alias, UPSTREAM_EVIDENCE_BRIEF_MATCHED_TERM_CHARS))
            .collect::<Vec<_>>(),
        "evidence_id": &object.evidence_id,
        "source_title": trim_text(&object.source_title, UPSTREAM_EVIDENCE_BRIEF_SOURCE_TITLE_CHARS),
        "source_cues": public_source_cues_for_title(&object.source_title)?,
        "text_cue": relation_open_object_text_cue(object),
    }))
}

fn relation_open_object_text_cue(object: &question_frame::RelationOpenObjectSupport) -> String {
    let cue = object.text_cue.trim();
    if !cue.is_empty() {
        return trim_text(cue, UPSTREAM_EVIDENCE_BRIEF_TEXT_CHARS);
    }
    trim_text(&object.text, UPSTREAM_EVIDENCE_BRIEF_TEXT_CHARS)
}
...
fn apply_evidence_tier_review(
    review: &mut ReviewRecord,
    claim_evidence_map: &[ClaimEvidenceMap],
    tiered_evidence_bindings: &[TieredEvidenceBinding],
) {
    if tiered_evidence_bindings.is_empty() {
        return;
    }
    let bindings_by_claim_evidence = tiered_evidence_bindings
        .iter()
        .map(|binding| ((binding.claim_index, binding.evidence_id.as_str()), binding))
        .fold(
            BTreeMap::<(usize, &str), &TieredEvidenceBinding>::new(),
            |mut map, (claim_evidence_key, binding)| {
                map.insert(claim_evidence_key, binding);
                map
            },
        );
    for claim in claim_evidence_map {
        if claim.evidence_ids.is_empty() {
            push_unique_text(&mut review.issues, "部分结论缺少证据引用，不能进入回答。");
            continue;
        };
        let mut claim_bindings = Vec::new();
        for evidence_id in &claim.evidence_ids {
            let Some(binding) =
                bindings_by_claim_evidence.get(&(claim.claim_index, evidence_id.as_str()))
            else {
                push_unique_text(
                    &mut review.issues,
                    "部分结论缺少 evidence tier 绑定，不能进入回答。",
                );
                continue;
            };
            claim_bindings.push(*binding);
            if !evidence_binding_source_scope_ready(binding) {
                push_unique_text(
                    &mut review.issues,
                    "部分结论的 source scope 未完成绑定，已要求降级。",
                );
            }
            if !evidence_binding_text_cue_ready(binding) {
                push_unique_text(
                    &mut review.issues,
                    "部分结论缺少可公开复核的 text cue，已要求降级。",
                );
            }
            if !evidence_binding_wording_strength_ready(binding) {
                push_unique_text(
                    &mut review.issues,
                    "部分结论的证据层级与公开措辞强度不一致，已要求降级。",
                );
            }
            if evidence_binding_is_answer_basis(binding) && !evidence_binding_gate_passed(binding) {
                push_unique_text(
                    &mut review.issues,
                    "部分结论的证据层级门槛未通过，不能作为直接结论。",
                );
            }
        }
        ...
        let has_answer_basis = claim_bindings
            .iter()
            .any(|binding| evidence_binding_is_answer_basis(binding));
        if !has_answer_basis {
            push_unique_text(
                &mut review.issues,
                "部分结论只命中补充线索，缺少可直接支撑结论的证据。",
            );
        }
    }
    if !review.issues.is_empty() {
...
fn evidence_binding_is_answer_basis(binding: &TieredEvidenceBinding) -> bool {
    binding.answer_use == "stable_basis" || binding.answer_use == "request_bound_basis"
}

fn evidence_binding_gate_passed(binding: &TieredEvidenceBinding) -> bool {
    binding.evidence_gate.get("status").and_then(Value::as_str) == Some("passed")
}

fn evidence_binding_required_field_missing(
    binding: &TieredEvidenceBinding,
    required_field: &str,
) -> bool {
    binding
        .evidence_gate
...
fn evidence_binding_source_scope_ready(binding: &TieredEvidenceBinding) -> bool {
    !binding.source_scope_policy_sha256.trim().is_empty()
        && !evidence_binding_required_field_missing(binding, "source_scope_policy")
}

fn evidence_binding_text_cue_ready(binding: &TieredEvidenceBinding) -> bool {
    !binding.text_cue.trim().is_empty()
        && !evidence_binding_required_field_missing(binding, "text_cue")
}

fn evidence_binding_wording_strength_ready(binding: &TieredEvidenceBinding) -> bool {
    matches!(
        binding.answer_use.as_str(),
        "stable_basis" | "request_bound_basis" | "supplemental_only"
    ) && binding
        .evidence_gate
        .get("decision_answer_use")
        .and_then(Value::as_str)
        == Some(binding.answer_use.as_str())
        && binding
            .evidence_gate
            .get("decision_evidence_tier")
            .and_then(Value::as_str)
...
pub fn local_answer(question: &str, package: &EvidencePackage) -> String {
    ...
    let basis_package = answer_basis_package(package);
    if basis_package.cards.is_empty() {
        return supplemental_only_answer(question, package);
    }
    ...
    answer.push_str(&answer_basis_intro(&basis_package));
    answer.push_str("目前能支持回答的主要材料如下，结论只限于这些文本直接能说明的范围。\n\n");
    if cards_include_later_forty(&basis_package.cards) {
        answer.push_str(
            "注意：以下包含第八十一回及以后（后四十回）材料；这类材料必须显式标注为后四十回内容，未标注时不能作为证据或参考。\n\n",
        );
    }
    let display_cards = answer_display_cards(&basis_package.question, &basis_package.cards, 4);
...
fn answer_basis_package(package: &EvidencePackage) -> EvidencePackage {
    ...
    let mut basis_package = package.clone();
    basis_package.cards = package
        .cards
        .iter()
        .filter(|card| answer_basis_ids.contains(&card.evidence_id))
        .cloned()
        .collect();
    basis_package
}
...
fn answer_basis_intro(package: &EvidencePackage) -> String {
    let answer_uses = package
        ...
        .map(|binding| binding.answer_use.as_str())
        .collect::<BTreeSet<_>>();
    if answer_uses == BTreeSet::from(["request_bound_basis"]) {
        return "就本次检索到的可追溯材料看，可以这样回答：\n\n".to_string();
    }
    if answer_uses.contains("stable_basis") && answer_uses.contains("request_bound_basis") {
        return "综合当前已登记资料和本次检索到的可追溯材料，可以这样回答：\n\n".to_string();
    }
    if package.knowledge_state_summary.human_marked_count > 0 {
        "人工标记资料显示，可以这样回答：\n\n".to_string()
    } else if package.knowledge_state_summary.runtime_usable_count > 0
        || answer_uses.contains("stable_basis")
    {
        "基于当前已登记资料，可以这样回答：\n\n".to_string()
    } else {
        "根据目前可检索到的文本，可以这样回答：\n\n".to_string()
...
fn supplemental_evidence_cards(package: &EvidencePackage) -> Vec<&EvidenceCard> {
    ...
    let supplemental_ids = package
        .tiered_evidence_bindings
        .iter()
        .filter(|binding| binding.answer_use == "supplemental_only")
        .map(|binding| binding.evidence_id.as_str())
        .collect::<BTreeSet<_>>();
    package
        .cards
        .iter()
        .filter(|card| supplemental_ids.contains(card.evidence_id.as_str()))
        .collect()
}
...
fn supplemental_only_answer(question: &str, package: &EvidencePackage) -> String {
    ...
    let mut answer = String::new();
    answer.push_str("目前只命中可作为补充线索的文本片段，还不能直接支撑结论。可先记录这些线索：");
    for (index, card) in supplemental.iter().take(3).enumerate() {
        answer.push_str(&format!(
            "\n{}. {}：{}",
            index + 1,
            evidence_card_source_label(card),
            answer_evidence_excerpt(question, card)
        ));
    }
    answer
}
...
fn append_supplemental_evidence_section(
    ...
    answer.push_str("\n\n补充线索（不能单独支撑主结论）：");
    for (index, card) in supplemental.iter().take(2).enumerate() {
        answer.push_str(&format!(
            "\n{}. {}：{}",
            index + 1,
            evidence_card_source_label(card),
...
fn query_expansion_catalog() -> Result<QueryExpansionCatalog> {
    ...
    let cache =
        QUERY_EXPANSION_CATALOG_CACHE.get_or_init(|| Mutex::new(default_query_expansion_cache()));
    let mut cache = rule_catalog::lock_rule_cache(cache, "query expansion")?;
    cache.catalog(
...
fn query_expansion_catalog_metadata() -> Result<Value> {
    let path = rule_catalog::configured_path(QUERY_EXPANSIONS_PATH_ENV);
    let cache =
        QUERY_EXPANSION_CATALOG_CACHE.get_or_init(|| Mutex::new(default_query_expansion_cache()));
    let mut cache = rule_catalog::lock_rule_cache(cache, "query expansion")?;
    ...
    Ok(cache.metadata(QUERY_EXPANSIONS_SCHEMA_VERSION, &catalog.catalog_version))
}

fn default_query_expansion_cache() -> rule_catalog::RuleFileCache<QueryExpansionCatalog> {
    rule_catalog::RuleFileCache::new(default_query_expansion_catalog())
}

fn default_query_expansion_catalog() -> QueryExpansionCatalog {
    parse_query_expansion_catalog(DEFAULT_QUERY_EXPANSIONS_JSON)
...
fn validate_query_expansion_catalog_source(source: &str) -> Result<()> {
    parse_query_expansion_catalog(source).map(|_| ())
}
...
pub fn domain_retrieval_plan(
    ...
    domain_retrieval_plan_from_parts(
        ...
        primary_intent,
        secondary_intents,
        required_evidence_types.to_vec(),
...
pub(crate) fn extract_chapter_no(title: &str) -> Option<i64> {
