use crate::{
    EvidenceCard, PromotionManifestInput, append_runtime_audit_event, bounded_optional_text,
    canonical_json_value,
    corpus_topology::{
        FullTextSearchRequest, card_matches_full_text_search_request,
        default_full_text_search_requests, full_text_search_request_id,
        full_text_search_requests_from_retrieval_repair_queries, parse_full_text_search_request,
    },
    evidence_card_source_layer, hash_text, now_rfc3339, question_frame,
    record_online_learning_promotion_manifest, relation_support_terms,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OnlineEvidenceCardSearchRequestRecord {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(crate) struct RehydratedStagedSourceHit {
    pub staged_card_id: String,
...
pub fn init_schema(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS online_evidence_search_requests (
            ...
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            UNIQUE(update_request_id, request_json)
        );
        ...
        CREATE INDEX IF NOT EXISTS idx_online_evidence_search_trace
            ON online_evidence_search_requests(trace_id, created_at);
        CREATE INDEX IF NOT EXISTS idx_online_evidence_search_update
            ON online_evidence_search_requests(update_request_id, status);
...
pub fn create_online_evidence_card_update_request(
    ...
    if let Some(existing) = load_update_request_by_idempotency(conn, &idempotency_key)? {
        ...
        ensure_search_requests_for_update_request(conn, &existing)?;
        return Ok(existing);
    }
    ...
    ensure_search_requests_for_update_request(conn, &request)?;
    Ok(request)
}

pub fn add_online_evidence_retrieval_repair_search_requests(
    conn: &Connection,
    trace_id: &str,
    resolved_question: &str,
    question_frame: Option<Value>,
    source_scope_policy: Value,
    retrieval_repair_queries: &[Value],
) -> Result<usize> {
    init_schema(conn)?;
    if retrieval_repair_queries.is_empty() {
        return Ok(0);
    }
    let request = match load_latest_update_request_for_trace(conn, trace_id)? {
        Some(request) => request,
        None => create_online_evidence_card_update_request(
            conn,
            OnlineEvidenceCardUpdateRequestInput {
                ...
                coverage_gap_reason: "upstream_retrieval_repair_recommended".to_string(),
                source_scope_policy: source_scope_policy.clone(),
                recall_advice_ref: Some("agent_runtime_retrieval_repair".to_string()),
            },
        )?,
    };
    let requests = full_text_search_requests_from_retrieval_repair_queries(
        resolved_question,
        question_frame.as_ref(),
        &source_scope_policy,
        retrieval_repair_queries,
    )?;
    let inserted = insert_search_requests_for_update_request(conn, &request, requests)?;
    requeue_card_ingest_job_for_new_search_requests(conn, &request, inserted.len())?;
    Ok(inserted.len())
}
...
fn requeue_card_ingest_job_for_new_search_requests(
    conn: &Connection,
    request: &OnlineEvidenceCardUpdateRequestRecord,
    new_search_request_count: usize,
) -> Result<()> {
    if new_search_request_count == 0 {
        return Ok(());
    }
    let Some(job) = load_card_ingest_job_by_request(conn, &request.update_request_id)? else {
        ensure_card_ingest_job_for_request(conn, request)?;
        return Ok(());
    };
    if !matches!(
        job.status.as_str(),
        "completed" | "dead_letter" | "retry_wait"
    ) {
        return Ok(());
    }
    let now = now_rfc3339();
    ...
    conn.execute(
        ...
        SET status = 'queued',
            stage = 'new_search_requests_queued',
            leased_by = NULL,
            lease_until = NULL,
            heartbeat_at = NULL,
            attempt_count = 0,
            ...
            updated_at = ?2
        WHERE job_id = ?1
          AND status IN ('completed', 'dead_letter', 'retry_wait')
        "#,
        params![&job.job_id, &now],
    )?;
    conn.execute(
        "UPDATE online_evidence_card_update_requests SET status = 'queued', updated_at = ?2 WHERE update_request_id = ?1",
    ...
    insert_staged_card_event(
        ...
        StagedCardEventInput {
            ...
            event_type: "card_ingest_job_requeued",
            from_status: Some(&previous),
            to_status: Some("queued"),
            reason_code: "new_search_requests_queued",
            rule_id: Some(ONLINE_INGEST_SCHEMA_VERSION),
            payload: &json!({
                "job_id": &job.job_id,
                "new_search_request_count": new_search_request_count,
            }),
        },
    )
}

fn ensure_search_requests_for_update_request(
    conn: &Connection,
    request: &OnlineEvidenceCardUpdateRequestRecord,
) -> Result<Vec<OnlineEvidenceCardSearchRequestRecord>> {
    if !load_search_requests_for_update_request(conn, &request.update_request_id)?.is_empty() {
        return load_search_requests_for_update_request(conn, &request.update_request_id);
    }
    let requests = default_full_text_search_requests(
        &request.resolved_question,
        request.question_frame.as_ref(),
        &request.source_scope_policy,
    )?;
    insert_search_requests_for_update_request(conn, request, requests)
}

fn insert_search_requests_for_update_request(
    conn: &Connection,
    request: &OnlineEvidenceCardUpdateRequestRecord,
    requests: Vec<FullTextSearchRequest>,
) -> Result<Vec<OnlineEvidenceCardSearchRequestRecord>> {
    let mut records = Vec::new();
    for search_request in requests {
        let request_value = canonical_json_value(&serde_json::to_value(&search_request)?);
        let search_request_id =
            full_text_search_request_id(&request.update_request_id, &search_request)?;
        let now = now_rfc3339();
        let created = conn.execute(
            r#"
            INSERT OR IGNORE INTO online_evidence_search_requests (
                search_request_id, update_request_id, trace_id, request_source,
                request_json, status, result_count, created_at, updated_at
        ...
        insert_staged_card_event(
            ...
            StagedCardEventInput {
                ...
                staged_card_id: None,
                candidate_id: None,
                event_type: "online_evidence_search_request_created",
                from_status: None,
                to_status: Some("queued"),
                reason_code: search_request.reason.as_str(),
                rule_id: Some(ONLINE_INGEST_SCHEMA_VERSION),
                payload: &json!({
                    "search_request_id": &search_request_id,
                    "request_source": &search_request.request_source,
                    "corpus_ids": &search_request.corpus_ids,
                    "source_layers": &search_request.source_layers,
                    "chapter_start": search_request.chapter_start,
                    "chapter_end": search_request.chapter_end,
                    "search_term_count": search_request.search_terms.len(),
                }),
            },
        )?;
        if let Some(record) = load_search_request(conn, &search_request_id)? {
            records.push(record);
        }
    }
    Ok(records)
...
pub fn run_online_evidence_card_worker_once(
    ...
    for job in jobs {
        ...
        match process_update_request(conn, &job, &request, retrieval_limit) {
            Ok(processed) => {
                report.search_request_count += processed.search_request_count;
...
pub fn list_online_evidence_card_search_requests_for_trace(
    conn: &Connection,
    trace_id: &str,
    limit: usize,
) -> Result<Vec<OnlineEvidenceCardSearchRequestRecord>> {
    ...
    let mut stmt = conn.prepare(
        r#"
        SELECT search_request_id, update_request_id, trace_id, request_source,
               request_json, status, result_count, created_at, updated_at
        FROM online_evidence_search_requests
        WHERE trace_id = ?1
        ORDER BY created_at, search_request_id
        LIMIT ?2
        "#,
    )?;
    query_search_request_rows(&mut stmt, params![trace_id, limit])
}
...
pub(crate) fn rehydrate_staged_source_hits_for_frame(
    conn: &Connection,
    trace_id: &str,
    frame: Option<&question_frame::RuntimeQuestionFrame>,
    limit: usize,
) -> Result<Vec<RehydratedStagedSourceHit>> {
    init_schema(conn)?;
    let Some(frame) = frame else {
        return Ok(Vec::new());
    };
    let match_specs = staged_card_match_specs(frame)?;
    ...
    for spec in &match_specs {
        for staged in list_rehydratable_staged_cards(conn, spec, limit)? {
            if seen.insert(staged.staged_card_id.clone()) && spec.matches(&staged) {
                staged_cards.push(staged);
                if staged_cards.len() >= limit {
                    break;
                }
            }
        }
    ...
    for staged in staged_cards {
        if let Some(hit) = rehydrate_staged_source_hit(conn, trace_id, &staged)? {
            hits.push(hit);
        }
    }
    Ok(hits)
...
struct StagedCardMatchSpec {
    source_scope: String,
    slot_id: String,
    entities_key: Option<String>,
    required_entity_terms: Vec<String>,
}

impl StagedCardMatchSpec {
    fn matches(&self, staged: &CanonicalStagedCardRecord) -> bool {
        if staged.source_scope != self.source_scope || staged.slot_id != self.slot_id {
            return false;
        }
        if let Some(entities_key) = &self.entities_key {
            return &staged.entities_key == entities_key;
        }
        entity_terms_match_staged_card(&staged.entities, &self.required_entity_terms)
    }
}

fn staged_card_match_specs(
    frame: &question_frame::RuntimeQuestionFrame,
) -> Result<Vec<StagedCardMatchSpec>> {
    let mut specs = Vec::new();
    if frame.has_relation_object() {
        if let (Some(subject), Some(predicate), Some(object)) =
            (&frame.subject, &frame.predicate, &frame.object)
        {
            ...
            specs.push(StagedCardMatchSpec {
                source_scope: "question_frame_relation_scope".to_string(),
                slot_id: predicate.id.clone(),
                entities_key: Some(stable_hash(&entities)?),
                required_entity_terms: Vec::new(),
            });
        }
    } else if frame.has_open_relation_object()
        && let (Some(subject), Some(predicate)) = (&frame.subject, &frame.predicate)
    {
        specs.push(StagedCardMatchSpec {
            source_scope: "question_frame_relation_scope".to_string(),
            slot_id: predicate.id.clone(),
            entities_key: None,
            required_entity_terms: subject.identity_terms(),
        });
    }
    ...
    {
        specs.push(StagedCardMatchSpec {
            source_scope: "question_frame_attribute_scope".to_string(),
            slot_id: predicate.id.clone(),
            entities_key: None,
            required_entity_terms: subject.identity_terms(),
        });
...
fn entity_terms_match_staged_card(entities: &Value, required_terms: &[String]) -> bool {
    if required_terms.is_empty() {
        return false;
    }
    let staged_terms = entities
        .as_array()
        .into_iter()
        .flatten()
        .flat_map(|entity| {
            let mut terms = Vec::new();
            if let Some(canonical) = entity.get("canonical").and_then(Value::as_str) {
        ...
        .map(|term| crate::normalize_text(&term))
        .collect::<std::collections::BTreeSet<_>>();
    required_terms.iter().any(|term| {
        let normalized = crate::normalize_text(term);
        !normalized.trim().is_empty() && staged_terms.contains(&normalized)
    })
}

fn list_rehydratable_staged_cards(
    conn: &Connection,
    spec: &StagedCardMatchSpec,
    limit: usize,
) -> Result<Vec<CanonicalStagedCardRecord>> {
    let limit = i64::try_from(limit.clamp(1, 500)).unwrap_or(500);
    if let Some(entities_key) = &spec.entities_key {
        let mut stmt = conn.prepare(
            r#"
            SELECT staged_card_id, exact_span_key, claim_key, cluster_key, source_scope,
                   slot_id, entities_key, entities_json, polarity, modality,
            ...
            WHERE source_scope = ?1
              AND slot_id = ?2
              AND entities_key = ?3
              AND status IN ('staged', 'merged', 'validated')
            ORDER BY updated_at DESC, staged_card_id DESC
            LIMIT ?4
            "#,
        )?;
        return query_staged_card_rows(
            &mut stmt,
            params![&spec.source_scope, &spec.slot_id, entities_key, limit],
        );
    }
    let mut stmt = conn.prepare(
        r#"
    ...
    query_staged_card_rows(&mut stmt, params![&spec.source_scope, &spec.slot_id, limit])
}

fn rehydrate_staged_source_hit(
    conn: &Connection,
    trace_id: &str,
    staged: &CanonicalStagedCardRecord,
) -> Result<Option<RehydratedStagedSourceHit>> {
    let source_id = staged.evidence.source_id.as_str();
    let Some(current_source_hash) = source_snapshot_hash(conn, source_id)? else {
        record_staged_rehydration_rejected(
            conn,
            trace_id,
            staged,
    ...
    if current_source_hash != staged.source_hash {
        record_staged_rehydration_rejected(
            conn,
            trace_id,
            staged,
            ...
            &json!({
                "source_id": source_id,
                "expected_source_hash": &staged.source_hash,
                "current_source_hash": &current_source_hash,
            }),
        )?;
        return Ok(None);
    }
    let Some(block) = load_source_block(conn, source_id, &staged.evidence.block_id)? else {
        record_staged_rehydration_rejected(
            conn,
            trace_id,
            staged,
            ...
            &json!({
                "source_id": source_id,
                "block_id": &staged.evidence.block_id,
            }),
        )?;
        return Ok(None);
    };
    let staged_text_hash = staged
        .supporting_spans
        .first()
        .and_then(|span| span.get("text_sha256"))
        .and_then(Value::as_str)
        .map(ToString::to_string)
        .unwrap_or_else(|| hash_text(&staged.evidence.text));
    if hash_text(&staged.evidence.text) != staged_text_hash {
        record_staged_rehydration_rejected(
            conn,
            trace_id,
            staged,
            "staged_text_hash_mismatch",
            &json!({"expected_text_sha256": &staged_text_hash}),
        )?;
        return Ok(None);
    }
    if !source_block_contains_staged_text(&block.text, &staged.evidence.text) {
        record_staged_rehydration_rejected(
            conn,
            trace_id,
            staged,
            "source_span_text_missing",
            &json!({
                "source_id": source_id,
                "block_id": &staged.evidence.block_id,
                "text_sha256": &staged_text_hash,
            }),
        )?;
        return Ok(None);
    }
    let evidence_id = format!(
        ...
        &stable_hash(&json!({
            "trace_id": trace_id,
            "staged_card_id": &staged.staged_card_id,
            "source_hash": &current_source_hash,
            "text_sha256": &staged_text_hash,
        }))?[..32]
    );
    let mut card = staged.evidence.clone();
    card.evidence_id = evidence_id.clone();
    card.source_title = block.source_title;
    card.source_url = block.source_url;
    card.revision_id = block.revision_id;
    card.verification_status = "rehydrated_staged_source_backed".to_string();
    let source_span_ref = json!({
        "staged_card_id": &staged.staged_card_id,
        "source_id": source_id,
        "source_hash": &current_source_hash,
    ...
    insert_staged_card_event(
        ...
        StagedCardEventInput {
            ...
            staged_card_id: Some(&staged.staged_card_id),
            candidate_id: None,
            event_type: "staged_card_rehydrated_for_request",
            from_status: Some(&staged.status),
            to_status: Some("request_scoped_source_hit"),
            reason_code: "source_hash_and_text_verified",
            rule_id: Some(&staged.validator_version),
            payload: &json!({
    ...
    append_runtime_audit_event(
        conn,
        trace_id,
        "online_evidence_staged_card_rehydrated",
        &json!({
            "staged_card_id": &staged.staged_card_id,
            "evidence_id": &evidence_id,
            "source_id": source_id,
    ...
    Ok(Some(RehydratedStagedSourceHit {
        staged_card_id: staged.staged_card_id.clone(),
        card,
        source_hash: current_source_hash,
        source_span_ref,
    }))
...
struct SourceBlockForRehydration {
    source_title: String,
    source_url: String,
...
fn source_snapshot_hash(conn: &Connection, source_id: &str) -> Result<Option<String>> {
    ...
    .optional()
    .map(|value| value.filter(|hash| !hash.trim().is_empty()))
    .map_err(Into::into)
}
...
fn load_source_block(
    ...
    source_id: &str,
    block_id: &str,
) -> Result<Option<SourceBlockForRehydration>> {
    if !sqlite_table_exists(conn, "blocks")? {
        return Ok(None);
    }
    conn.query_row(
        r#"
        SELECT source_title, source_url, revision_id, text
        FROM blocks
...
fn source_block_contains_staged_text(source_text: &str, staged_text: &str) -> bool {
    let cue = staged_text
        .trim_start_matches("...")
        .trim_end_matches("...")
        .trim();
    ...
    source_text.contains(cue)
        || crate::normalize_text(source_text).contains(&crate::normalize_text(cue))
}

fn record_staged_rehydration_rejected(
    conn: &Connection,
    trace_id: &str,
    staged: &CanonicalStagedCardRecord,
    ...
    insert_staged_card_event(
        ...
        StagedCardEventInput {
            ...
            staged_card_id: Some(&staged.staged_card_id),
            candidate_id: None,
            event_type: "staged_card_rehydration_rejected",
            from_status: Some(&staged.status),
            to_status: Some("not_used"),
            reason_code,
            rule_id: Some(&staged.validator_version),
            payload,
        },
...
fn process_update_request(
    ...
    let search_requests = ensure_search_requests_for_update_request(conn, request)?;
    let search_requests = search_requests
        .into_iter()
        .filter(|search_request| search_request.status == "queued")
        .collect::<Vec<_>>();
    ...
    for search_request_record in search_requests {
        let cards = execute_full_text_search_request(
            conn,
            request,
        ...
        for card in cards {
            if !seen_blocks.insert(format!("{}:{}", card.source_id, card.block_id)) {
                continue;
            }
            let source_hash = source_hash_for_card(conn, &card)?;
            if let Some(candidate) = stage_candidate_from_frame(
                request,
                frame.as_ref(),
                card.clone(),
                source_hash.clone(),
                ...
                record_card_ingest_job_stage_ref(
                    conn,
                    job,
                    Some(&raw_candidate.candidate_id),
                    None,
                )?;
...
fn execute_full_text_search_request(
    conn: &Connection,
    update_request: &OnlineEvidenceCardUpdateRequestRecord,
    search_request_record: &OnlineEvidenceCardSearchRequestRecord,
    retrieval_limit: usize,
) -> Result<Vec<EvidenceCard>> {
    let request = parse_full_text_search_request(&search_request_record.request)?;
    let per_term_limit = retrieval_limit.saturating_mul(4).max(retrieval_limit);
    let mut cards_by_block = std::collections::BTreeMap::<String, EvidenceCard>::new();
    for term in &request.search_terms {
        let result =
            search_evidence_result(conn, term, per_term_limit, &request.required_evidence_types)?;
        for card in result.cards {
            if card_matches_full_text_search_request(&card, &request)? {
                cards_by_block
                    .entry(format!("{}:{}", card.source_id, card.block_id))
                    .or_insert(card);
            }
        }
    }
    let mut cards = cards_by_block.into_values().collect::<Vec<_>>();
    cards.truncate(retrieval_limit);
    let result_count = i64::try_from(cards.len()).unwrap_or(i64::MAX);
    let now = now_rfc3339();
    conn.execute(
        r#"
        UPDATE online_evidence_search_requests
        SET status = 'completed',
            result_count = ?2,
            updated_at = ?3
        WHERE search_request_id = ?1
        "#,
        params![&search_request_record.search_request_id, result_count, &now],
    )?;
    insert_staged_card_event(
        conn,
        StagedCardEventInput {
            trace_id: &update_request.trace_id,
            update_request_id: Some(&update_request.update_request_id),
            staged_card_id: None,
            candidate_id: None,
            event_type: "online_evidence_search_request_completed",
            from_status: Some(&search_request_record.status),
            to_status: Some("completed"),
            reason_code: "local_full_text_search_completed",
            rule_id: Some(ONLINE_INGEST_SCHEMA_VERSION),
            payload: &json!({
                "search_request_id": &search_request_record.search_request_id,
...
fn stage_candidate_from_frame(
    ...
    if let Some(candidate) =
        stage_relation_candidate_from_frame(request, frame, card.clone(), source_hash.clone())?
    {
        return Ok(Some(candidate));
    }
    if let Some(candidate) =
        stage_attribute_candidate_from_frame(request, frame, card.clone(), source_hash.clone())?
    {
        return Ok(Some(candidate));
    }
...
fn stage_attribute_candidate_from_frame(
    request: &OnlineEvidenceCardUpdateRequestRecord,
    frame: &question_frame::RuntimeQuestionFrame,
    card: EvidenceCard,
    source_hash: String,
    ...
    let Some(support) = question_frame::attribute_card_support(frame, &card) else {
        return Ok(None);
    };
    let entities = canonical_json_value(&json!([
        {"role": "subject", "canonical": subject.canonical, "aliases": subject.aliases},
        {"role": "attribute_value", "canonical": support.claim_value, "aliases": []}
    ]));
    let entities_key = stable_hash(&entities)?;
    Ok(Some(StageCandidateInput {
        update_request_id: request.update_request_id.clone(),
...
fn validate_and_promote_staged_card(
    ...
    let promotion_manifest = record_online_learning_promotion_manifest(
        conn,
        PromotionManifestInput {
            artifact_kind: "evidence".to_string(),
            candidate_ids: vec![record.staged_card_id.clone()],
            source_trace_refs: record.created_from_trace_ids.clone(),
            source_span_refs: json!(&record.supporting_spans),
            rule_diff_refs: json!([]),
            merge_conflict_decision: json!({
                "status": "passed",
                "decision": "merge_or_promote_canonical_staged_card",
                "claim_key": &record.claim_key,
                "exact_span_key": &record.exact_span_key,
                "cluster_key": &record.cluster_key,
                "source_hash": &record.source_hash,
            }),
            target_ref: "evidence_cards".to_string(),
            expected_version_bump: "data_store_promotion".to_string(),
            regression_cases: json!([{
                "suite_ref": "online_evidence_card_validator",
                "status": "passed",
                "validator_version": &record.validator_version,
                "source_hash": &record.source_hash,
            }]),
            reviewer_policy: json!({
                "policy": "local_source_hash_validator",
                "validator_version": &record.validator_version,
            ...
            dry_run_result: json!({
                ...
                "target_evidence_id": &promoted_id,
            }),
            rollback_ref: json!({
                "kind": "sqlite_row_revert",
                "target_store": "evidence_cards",
                "promoted_evidence_id": &promoted_id,
                "staged_card_id": &record.staged_card_id,
                "previous_staged_status": previous,
            }),
        },
    )?;
...
pub fn online_evidence_card_ingest_stats(conn: &Connection) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "search_requests": grouped_count_json(conn, "online_evidence_search_requests", "status")?,
...
fn load_search_request(
    conn: &Connection,
    search_request_id: &str,
) -> Result<Option<OnlineEvidenceCardSearchRequestRecord>> {
    let mut stmt = conn.prepare(
        r#"
        SELECT search_request_id, update_request_id, trace_id, request_source,
               request_json, status, result_count, created_at, updated_at
    ...
    let mut rows = query_search_request_rows(&mut stmt, params![search_request_id])?;
    Ok(rows.pop())
}

fn load_search_requests_for_update_request(
    conn: &Connection,
    ...
    query_search_request_rows(&mut stmt, params![update_request_id])
}

fn query_search_request_rows<P>(
    stmt: &mut rusqlite::Statement<'_>,
    params: P,
) -> Result<Vec<OnlineEvidenceCardSearchRequestRecord>>
where
    P: rusqlite::Params,
{
    ...
    rows.collect::<std::result::Result<Vec<_>, _>>()?
        .into_iter()
        .map(|row| {
            Ok(OnlineEvidenceCardSearchRequestRecord {
                search_request_id: row.0,
...
fn load_latest_update_request_for_trace(
    conn: &Connection,
    trace_id: &str,
) -> Result<Option<OnlineEvidenceCardUpdateRequestRecord>> {
    let mut stmt = conn.prepare(
        ...
        FROM online_evidence_card_update_requests
        WHERE trace_id = ?1
        ORDER BY created_at DESC, update_request_id DESC
        LIMIT 1
        "#,
    )?;
    query_optional_update_request(&mut stmt, params![trace_id])
}
