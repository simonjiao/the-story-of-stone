...
static GOVERNANCE_RULES_CATALOG_CACHE: OnceLock<Mutex<RuleFileCache<GovernanceRuleCatalog>>> =
    OnceLock::new();
...
fn governance_rule_catalog() -> Result<GovernanceRuleCatalog> {
    let path = configured_path(GOVERNANCE_RULES_PATH_ENV);
    let cache =
        GOVERNANCE_RULES_CATALOG_CACHE.get_or_init(|| Mutex::new(default_governance_rule_cache()));
    let mut cache = lock_rule_cache(cache, "governance")?;
    cache.catalog(
        GOVERNANCE_RULES_PATH_ENV,
        path,
        default_governance_rule_catalog(),
        parse_governance_rule_catalog,
    )
}

pub(crate) fn governance_rule_catalog_metadata() -> Result<Value> {
    let path = configured_path(GOVERNANCE_RULES_PATH_ENV);
    let cache =
        GOVERNANCE_RULES_CATALOG_CACHE.get_or_init(|| Mutex::new(default_governance_rule_cache()));
    let mut cache = lock_rule_cache(cache, "governance")?;
    ...
    Ok(cache.metadata(GOVERNANCE_RULES_SCHEMA_VERSION, &catalog.catalog_version))
}

fn default_governance_rule_cache() -> RuleFileCache<GovernanceRuleCatalog> {
    RuleFileCache::new(default_governance_rule_catalog())
}

fn default_governance_rule_catalog() -> GovernanceRuleCatalog {
    parse_governance_rule_catalog(DEFAULT_GOVERNANCE_RULES_JSON)
