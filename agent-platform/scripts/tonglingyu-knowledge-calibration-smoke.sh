...
# This is a runtime governance smoke, not a knowledge-base build/import path.
# It creates a tiny synthetic candidate in a temporary runtime DB and verifies
# the knowledge-calibration state machine without loading source snapshots.
RUN_ID="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_ID:-local-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
ARTIFACT_ROOT="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_ARTIFACT_ROOT:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_ARTIFACT_ROOT:-${REPO_ROOT}/data/tonglingyu/knowledge-calibration-smoke}}"
ARTIFACT_DIR="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_ARTIFACT_DIR:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_ARTIFACT_DIR:-${ARTIFACT_ROOT}/${RUN_ID}}}"
DB_PATH="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_DB_PATH:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_DB_PATH:-${ARTIFACT_DIR}/knowledge-calibration.db}}"
INPUT_PATH="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_INPUT_PATH:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_INPUT_PATH:-${ARTIFACT_DIR}/calibration-input.json}}"
REPORT_PATH="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_REPORT_PATH:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_REPORT_PATH:-${ARTIFACT_DIR}/calibration-report.json}}"
SUMMARY_PATH="${TONGLINGYU_KNOWLEDGE_CALIBRATION_SMOKE_SUMMARY_PATH:-${TONGLINGYU_KNOWLEDGE_CALIBRATION_SUMMARY_PATH:-${ARTIFACT_DIR}/calibration-smoke-summary.json}}"
...
try:
    ...
    marker = "calibration-smoke"
    payload = {
        "claim": f"sample governance calibration item {marker}",
        "marker": marker,
    }
    source_refs = [f"runtime://tonglingyu/governance_tasks/{marker}"]
    evidence_refs = [f"runtime://tonglingyu/governance_evidence/{marker}"]
    ...
    run_input = {
        ...
        "eval_context": None,
        "input_kind": "governance_task",
        "input_ref": f"runtime://tonglingyu/governance_tasks/{marker}",
        "item_id": item_id,
        "llm_config": None,
        "llm_judgement": None,
        "method": "rule",
        "rqa_context": None,
        "rule_context": {
            "block_id": f"runtime/governance/{marker}",
            "exact_terms": [marker],
            "required_evidence_type": "governance_task",
            "source_id": "external-runtime-db",
            "usage_boundary": "synthetic runtime calibration smoke, not human marked",
            "version_boundary": "external runtime DB release boundary",
        },
...
summary = {
    "object": "tonglingyu.knowledge_calibration_smoke",
    "schema_version": 1,
