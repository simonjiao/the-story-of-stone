...
--full     Quick gates plus cargo clippy and cargo test.
EOF
}
...
run uv run --no-sync python -m py_compile "${REPO_DIR}/scripts/version.py"
...
if [[ "${MODE}" == "full" ]]; then
  ...
  run cargo test \
    --manifest-path "${REPO_DIR}/agent-platform/Cargo.toml" \
    --workspace
  if [[ "${TONGLINGYU_QA_COMPOSE_CONFIG:-false}" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
    if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
      ...
      printf 'SKIP docker compose config command not found\n'
    fi
  else
    printf 'SKIP docker compose config set TONGLINGYU_QA_COMPOSE_CONFIG=true to enable\n'
  fi
fi
